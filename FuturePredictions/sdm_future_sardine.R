...
# Load static data
port_codes <- fread("C:/GitHub/EconAnalysis/Data/Ports/Ports.landing.FF.csv")
...
dist_list <- lapply(1:nrow(port_locs), function(i) {
  ...
  dists <- distHaversine(matrix(c(grid$lon, grid$lat), ncol = 2),
                         c(port$Longitude, port$Latitude)) / 1000
  data.table(
    lon = grid$lon,
    lat = grid$lat,
    PORT_NAME = port$PORT_NAME,
    AGENCY_CODE = port$AGENCY_CODE,
    dist = dists
  )
})
...
years <- 2070:2090
months <- 1:12
year_months <- CJ(year = years, month = months)

with_progress({
  results <- future_map(year_months, function(ym) {
