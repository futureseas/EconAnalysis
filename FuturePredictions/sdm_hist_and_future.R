...
dat <- nc_open("C:\\Data\\Future Projections CPS SC-GAMs ROMS Domain\\albSDMs_proj_1_2070.nc")
...
for (y in 2070:2070) {
  for (m in 1:12) {
    for (j in 1:nrow(ports)) {
      # Read netcdf
      dat <- nc_open(paste0("C:\\Data\\Future Projections CPS SC-GAMs ROMS Domain\albSDMs_proj_1_", paste0(as.character(m), paste0("_", as.character(y)))))
