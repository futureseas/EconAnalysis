...
with_progress({
  results_list <- future_map(
    ...
    function(ym) {
      y <- ym$year
      m <- ym$month
      nc_path <- sprintf("C:/Data/Future Projections CPS SC-GAMs ROMS Domain/squid/squidSDMs_proj_%d_%d.nc", m, y)
...
fwrite(SDM_all, here::here("FuturePredictions", "SDM", "MSQD_FutureSDM_port_day.csv"))
