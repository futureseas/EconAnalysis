...
with_progress({
  results_list <- future_map(
    ...
    function(ym) {
      y <- ym$year
      m <- ym$month
      nc_path <- sprintf("C:/Data/Future Projections CPS SC-GAMs ROMS Domain/albacore/albSDMs_proj_%d_%d.nc", m, y)
      ...
      result <- sdm_dt[, .(
        SDM_90_GFDL = mean(GFDL, na.rm = TRUE),
        SDM_90_IPSL = mean(IPSL, na.rm = TRUE),
        SDM_90_HADL = mean(HADL, na.rm = TRUE)
...
fwrite(SDM_all, here::here("FuturePredictions", "SDM", "ALBC_FutureSDM_port_day.csv"))
