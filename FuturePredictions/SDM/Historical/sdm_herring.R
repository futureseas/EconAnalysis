...
with_progress({
  results_list <- future_map(
    ...
    function(ym) {
      y <- ym$year
      m <- ym$month
      nc_path <- sprintf("C:/Data/Future Projections CPS SC-GAMs ROMS Domain/herring/herrSDMs_proj_%d_%d.nc", m, y)
...
fwrite(SDM_all, here("FuturePredictions", "SDM", "HERR_FutureSDM_port_day.csv"))
