...
with_progress({
  results_list <- future_map(
    ...
    function(ym) {
      y <- ym$year
      m <- ym$month
      nc_path <- sprintf("C:/Data/Historical CPS SC-GAMs ROMS Domain/chubMackerel/chub_%d_%d_mboost_roms.nc", m, y)
...
fwrite(SDM_all, here("FuturePredictions", "SDM", "Historical", "CMCK_SDM_port_day.csv"))
