...
transformed parameters{
  real mu_qPSDN[N];
  for (n in 1:N) {
      mu_qPSDN[n] = const_psdn[portID[n]] + prob_psdn[portID[n]] * sdm_psdn[n] + prob_msqd[portID[n]] * sdm_msqd[n]; //+ year_psdn[yearID[n]];
  }
}
...
// Priors
  for (l in 1:L) {
   prob_msqd[l] ~ normal(0, sd_probMSQD);
...
// Posterior
  for (n in 1:N) {
    if (qPSDN[n] == 0)
      ...
      qPSDN[n] ~ lognormal(mu_qPSDN[n], sd_qPSDN);
    }
  }
}
