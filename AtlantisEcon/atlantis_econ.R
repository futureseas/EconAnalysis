...
options(scipen=999)
PacFIN.month.Atlantis <- PacFIN.month %>% dplyr::filter(PACFIN_SPECIES_CODE!='OTHER') %>%
  ...
  dplyr::filter(PORT_AREA_CODE == "SDA" | PORT_AREA_CODE == "LAA" | PORT_AREA_CODE == "SBA" | 
                  ...
                  PORT_AREA_CODE == "NPS") %>% 
    group_by(PORT_AREA_CODE, LANDING_YEAR, PACFIN_SPECIES_CODE) %>%
    summarise(LANDINGS = sum(LANDED_WEIGHT_MTONS.sum))

write.csv(PacFIN.month.Atlantis, 
          here::here("AtlantisEcon", "PacFIN_year_atlantis.csv"), 
          row.names = FALSE)
