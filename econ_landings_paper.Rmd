...
PacFIN.month.CPS <- PacFIN.month %>% filter(PACFIN_SPECIES_CODE %in% c("PSDN", "MSQD", "NANC", "PHRG"))
...
coeff <- 100000
  ...
  g1 <- ggplot(df, aes(x=LANDING_YEAR, y = value, colour = Variable)) + geom_line(size=1) +
    ...
    theme(plot.title = element_text(size=9, face="bold.italic"), 
          axis.text = element_text(size = 7), axis.title = element_text(size = 8)) + 
      ggtitle("(a) Market Squid")  +  scale_color_brewer(palette="Set2")
...
coeff <- 100000
  ...
  g2 <- ggplot(df, aes(x=LANDING_YEAR, y = value, colour = Variable)) + geom_line(size=1) +
    ...
    theme(plot.title = element_text(size=9, face="bold.italic"), 
          axis.text = element_text(size = 7), axis.title = element_text(size = 8)) + 
      ggtitle("(b) Pacific Sardine")  +  scale_color_brewer(palette="Set2")
...
coeff <- 30000
  ...
  scale_color_brewer(palette="Set2", labels=c("AFI_PRICE_PER_KG.mean.mean" = "Price", 
                                              "LANDED_WEIGHT_MTONS.sum.sum" = "Landings"))
...
coeff <- 0.001
  ...
  df$LANDED_WEIGHT_MTONS.sum.sum <- df$LANDED_WEIGHT_MTONS.sum.sum * coeff 
  df <- gather(df, key = Variable, value = value,
               c("LANDED_WEIGHT_MTONS.sum.sum", "n_vessel"))
  g1 <- ggplot(df, aes(x=LANDING_YEAR, y = value, colour = Variable)) + geom_line(size=1) +
    scale_y_continuous(name = "Number of Vessels", sec.axis = sec_axis(~./coeff, name = "")) +
    theme(legend.position="none") + scale_x_continuous(name = element_blank()) + 
    theme(plot.title = element_text(size=9, face="bold.italic"), 
          axis.text = element_text(size = 7), axis.title = element_text(size = 8)) + 
      ggtitle("(a) Market Squid")  +  scale_color_brewer(palette="Set2")
...
coeff <- 0.001
  ...
  df <- gather(df, key = Variable, value = value,
               c("LANDED_WEIGHT_MTONS.sum.sum", "n_vessel"))
  g2 <- ggplot(df, aes(x=LANDING_YEAR, y = value, colour = Variable)) + geom_line(size=1) +
    scale_y_continuous(name = "", sec.axis = sec_axis(~./coeff, name = "Landings (M Tons)")) + 
    theme(legend.position="none") + scale_x_continuous(name = "") +
    theme(plot.title = element_text(size=9, face="bold.italic"), 
          axis.text = element_text(size = 7), axis.title = element_text(size = 8)) + 
      ggtitle("(b) Pacific Sardine")  +  scale_color_brewer(palette="Set2")
...
coeff <- 0.001
  ...
  g4 <- ggplot(df, aes(x=LANDING_YEAR, y = value, colour = Variable)) + geom_line(size=1) +
    ...
    theme(plot.title = element_text(size=9, face="bold.italic"), 
          axis.text = element_text(size = 7), axis.title = element_text(size = 8)) + 
    ggtitle("(d) Northern Anchovy") + guides(colour=guide_legend(title="Variables: ")) + 
  scale_color_brewer(palette="Set2", labels=c("n_vessel" = "Number of vessels", 
                                              "LANDED_WEIGHT_MTONS.sum.sum" = "Landings"))
...
Substitution between species is likely to be related with the gear that a vessel or fisher have already invest for their fishing activities. However, a vessel is not restricted to an specific gear. They can switch between different alternatives. In average a vessel use 2.96 gears for their fishing activities ir out sample data, with an standard deviation of 1.96. If we only focus in CPS species, a vessel switch between 1.39 gears in average to harvest market squid, 1.1 gears in average to harvest Northern anchovy, 1.18 gears in average to harvest Pacific herring and 1.21 in average to harvest Pacific sardine. In terms of gears, midwater trawl and seine allow for the highest level of sustitution between CPS species. In average, a vessel using midwater trawl harvest 2.37 CPS species, while a vessel using seine harvest in average 1.92 CPS species (see Table \ref{n_CPS_gear})

```{r n_gear_used_vessels, eval=FALSE, include=FALSE}
# How many gears a vessel use in the dataset
PacFIN.CPS.unique.gear <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_GEAR_CODE) %>% 
  unique() 
  PacFIN.CPS.n_gear <- PacFIN.CPS.unique.gear %>% mutate(N_gear = 1) %>% group_by(VESSEL_NUM) %>% summarise(N_gear= sum(N_gear)) 
...
PacFIN.Gear.PSDN <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("PSDN")) %>% mutate(N_gear = 1) %>%  group_by(VESSEL_NUM) %>% 
  summarise(N_gear = sum(N_gear)) %>% mutate(Species = "PSDN")
PacFIN.Gear.MSQD <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("MSQD")) %>% mutate(N_gear = 1) %>%  group_by(VESSEL_NUM) %>% 
  summarise(N_gear = sum(N_gear)) %>% mutate(Species = "MSQD")
PacFIN.Gear.NANC <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("NANC")) %>% mutate(N_gear = 1) %>%  group_by(VESSEL_NUM) %>% 
  summarise(N_gear = sum(N_gear)) %>% mutate(Species = "NANC")
PacFIN.Gear.PHRG <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("PHRG")) %>% mutate(N_gear = 1) %>%  group_by(VESSEL_NUM) %>% 
  summarise(N_gear = sum(N_gear)) %>% mutate(Species = "PHRG")
PacFIN.Gear <- rbind.data.frame(PacFIN.Gear.PSDN, PacFIN.Gear.NANC, PacFIN.Gear.MSQD, PacFIN.Gear.PHRG)

table <- xtabs(N_gear ~ Species, aggregate(N_gear ~ Species, PacFIN.Gear, mean))
#table
...
PacFIN.CPS <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% 
...
Seine is the most used gear to harvest Pacific sardine, Northern anchovy and Market squid. From Table \ref{Table:n_vess_gear} we can observe that 35\% vessels have used seine to harvest Market squid, 52\% to harvest Pacific sardine and 56\% to harvest Northern anchovy. In the case of Pacific herring, vessels prefer to use gill net or midwater trawl for their harvest. Similar patter is observed when we compute the total landed weight for these four species during the period 2000-2020, presented in Table \ref{table:landing_by_gears}.

```{r n_vessels_using_gear, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
PacFIN.Gear.PSDN <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("PSDN")) %>% mutate(N_vessels = 1) %>%  group_by(PACFIN_GEAR_CODE) %>% 
  summarise(N_vessels = sum(N_vessels)) %>% mutate(Species = "PSDN") %>% filter(PACFIN_GEAR_CODE %in% 
   c("DPN", "GLN", "MDT", "ONT", "POL", "SEN"))

PacFIN.Gear.MSQD <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("MSQD")) %>% mutate(N_vessels = 1) %>%  group_by(PACFIN_GEAR_CODE) %>% 
  summarise(N_vessels = sum(N_vessels)) %>% mutate(Species = "MSQD") %>% filter(PACFIN_GEAR_CODE %in% 
   c("DPN", "GLN", "MDT", "ONT", "POL", "SEN"))

PacFIN.Gear.PHRG <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("PHRG")) %>% mutate(N_vessels = 1) %>%  group_by(PACFIN_GEAR_CODE) %>% 
  summarise(N_vessels = sum(N_vessels)) %>% mutate(Species = "PHRG") %>% filter(PACFIN_GEAR_CODE %in% 
   c("DPN", "GLN", "MDT", "ONT", "POL", "SEN"))

PacFIN.Gear.NANC <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("NANC")) %>% mutate(N_vessels = 1) %>%  group_by(PACFIN_GEAR_CODE) %>% 
  summarise(N_vessels = sum(N_vessels)) %>% mutate(Species = "NANC") %>% filter(PACFIN_GEAR_CODE %in% 
   c("DPN", "GLN", "MDT", "ONT", "POL", "SEN"))
...
table <- round(xtabs(LANDED_WEIGHT_LBS.sum ~  PACFIN_GEAR_CODE + PACFIN_SPECIES_CODE, data = PacFIN.CPS )/2204.6, 0 )
# table

colnames(table) = c("Market squid", "Northern Anchovy", "Pacific Herring", "Pacific Sardine") 
rownames(table) = c("Dip Net", "Gill Net", "Midwater Trawl", "Other Net Gear", "Pole", "Seine") 
print(xtable(table, caption = 'Landings of CPS species by gear used. \\label{table:landing_by_gears}', type = "latex"), comment=FALSE,  caption.placement = "top")
...
In average, a vessel land in 4.19 ports, with an standard deviation of 3.39. 

```{r n_ports_vessels, eval=FALSE, include=FALSE}
# How many gears a vessel use in the dataset
PacFIN.CPS.unique.port <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PORT_NAME) %>% 
  unique() 
...
PacFIN.Gear.PSDN <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PORT_NAME) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("PSDN")) %>% mutate(N_port = 1) %>%  group_by(VESSEL_NUM) %>% 
  summarise(N_port = sum(N_port)) %>% mutate(Species = "PSDN")
PacFIN.Gear.MSQD <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PORT_NAME) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("MSQD")) %>% mutate(N_port = 1) %>%  group_by(VESSEL_NUM) %>% 
  summarise(N_port = sum(N_port)) %>% mutate(Species = "MSQD")
PacFIN.Gear.NANC <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PORT_NAME) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("NANC")) %>% mutate(N_port = 1) %>%  group_by(VESSEL_NUM) %>% 
  summarise(N_port = sum(N_port)) %>% mutate(Species = "NANC")
PacFIN.Gear.PHRG <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PORT_NAME) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("PHRG")) %>% mutate(N_port = 1) %>%  group_by(VESSEL_NUM) %>% 
...
collapse <- summaryBy(Landings ~ Species_name + Species_code + Port + State, FUN=meanfun, data=PacFIN_yearly) %>%
  filter(Port == "BDA" | Port == "CBA" | Port == "CCA" | Port == "CLO" | Port == "ERA" | Port == "LAA" | Port == "MNA" | Port == "MRA" | Port == "NPA" | Port == "NPS" | Port == "SBA" | Port == "SDA" | Port == "SFA") %>%
  filter(Species_name == "PACIFIC SARDINE" | Species_name == "NORTHERN ANCHOVY" | Species_name == "MARKET SQUID" | Species_name == "PACIFIC HERRING") %>%
  mutate(Port = fct_relevel(Port, "SDA", "LAA", "SBA", "MNA", "MRA", "SFA", "BDA", "ERA", "CCA", "CBA", "NPA", "CLO", "NPS"))
...
```{r ts_by_port_price,  fig.cap = "Total annual landing by port area. 2000 - 2020. \\textit{Notes:} CLO = Columbia River (OR); LAA = Los Angeles; MNA = Monterey; SBA = Santa Barbara; SFA = San Francisco.\\label{fig:ts_price_by_ports}" , echo=FALSE, message=FALSE, warning=FALSE}
...
ggplot(collapse, aes(y=Price, x=Landing_year, color=Species_code, group=Species_code)) +
  geom_line(size=1) + xlab("Port areas") + ylab("Price ($/tons)") + 
  facet_wrap(~ Port, labeller = port_names) +
  theme(legend.position="bottom") + 
  theme(axis.text.x = element_text(angle=90)) +
  guides(color=guide_legend(title="Species: ")) +
...
```{r ts_n_vessels, echo=FALSE, fig.cap="Total number of vessel by species. 2000 - 2020.\\label{fig:ts_n_vessels}", message=FALSE, warning=FALSE}
collapse <- PacFIN_yearly %>%
...
# Clean dataset
PacFIN_yearly = PacFIN_yearly[,!grepl("*_LE_",names(PacFIN_yearly))]
PacFIN_yearly <- PacFIN_yearly %>%
  filter(Species_code == "PSDN" | Species_code == "MSQD"  | Species_code == "NANC") %>%
...
est_data = subset(PacFIN_yearly, select = -c(MSQD_PSDN_SDM_60, MSQD_NANC_SDM_20, MSQD_ACL_PSDN.sum, 
