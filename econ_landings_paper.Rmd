...
collapse <- PacFIN_dat %>%
  ...
  select(Landing_year, Port, PSDN_SDM_60, Landings) %>% 
...
fit_qPSDN <- brm(
  bf(PSDN_Landings ~ PSDN_SDM_60 + MSQD_SDM_90 + NANC_SDM_60 + s(ACL_PSDN) + (1 + PSDN_SDM_60 + MSQD_SDM_90 + NANC_SDM_60 | port_ID),
                hu ~ PSDN_SDM_60 + MSQD_SDM_90 + NANC_SDM_60 + s(ACL_PSDN) + (1 + PSDN_SDM_60 + MSQD_SDM_90 + NANC_SDM_60 | port_ID)),
                          data = est_data_psnd,
                          family = hurdle_gamma(), 
...
```{r psdn_est_v2, message=FALSE, warning=FALSE, include=FALSE}
fit_qPSDN_90 <- brm(
...
pp_check(fit_qPSDN) + ggtitle('Pacific Sardine model') + theme(legend.position = "bottom")
```

### Effect of SDM's on Sardine landings

```{r int_effect_smooth, eval=FALSE, include=FALSE}
pseq_gamma <- plot(conditional_smooths(fit_qPSDN), plot = FALSE, ask = FALSE)
...
plot(conditional_effects(fit_qPSDN, "PSDN_SDM_60", surface=TRUE), ask = FALSE)
```  

```{r MSQD_effect_total, echo=FALSE, message=FALSE, warning=FALSE}
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qPSDN, "MSQD_SDM_90", surface=TRUE), ask = FALSE)
```  

```{r NANC_effect_total, echo=FALSE, message=FALSE, warning=FALSE}
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qPSDN, "NANC_SDM_60", surface=TRUE), ask = FALSE)
```  

```{r ACL_effect, echo=FALSE, message=FALSE, warning=FALSE}
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qPSDN, "ACL_PSDN", surface=TRUE), ask = FALSE)
```  
...
plot(conditional_effects(
  fit_qPSDN, "PSDN_SDM_60", surface=TRUE, conditions = conditions, 
...
```{r by_port_msqd_sdm}
plot(conditional_effects(
  fit_qPSDN, "NANC_SDM_60", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
```
...
plot(conditional_effects(
  fit_qPSDN, "PSDN_SDM_60:MSQD_SDM_90", surface=TRUE), ask = FALSE)
```   

```{r int_effect_NANC_total, echo=FALSE, message=FALSE, warning=FALSE}
plot(conditional_effects(
  fit_qPSDN, "PSDN_SDM_60:NANC_SDM_60", surface=TRUE), ask = FALSE)
```   

```{r int_effect_MSQD_by_port, echo=FALSE, message=FALSE, warning=FALSE}
plot(conditional_effects(
  fit_qPSDN, "PSDN_SDM_60:MSQD_SDM_90", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
```

```{r int_effect_NANC_by_port, echo=FALSE, message=FALSE, warning=FALSE}
plot(conditional_effects(
  fit_qPSDN, "PSDN_SDM_60:NANC_SDM_60", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
```
...
fit_qMSQD <- brm(
 bf(MSQD_Landings ~ MSQD_SDM_90 + PSDN_SDM_60_dOpen + NANC_SDM_60 + dClose + (1 + MSQD_SDM_90 + PSDN_SDM_60_dOpen + NANC_SDM_60 + dClose | port_ID),
               hu ~ MSQD_SDM_90 + PSDN_SDM_60_dOpen + NANC_SDM_60 + dClose + (1 + MSQD_SDM_90 + PSDN_SDM_60_dOpen + NANC_SDM_60 + dClose | port_ID)),
...
pp_check(fit_qMSQD) + ggtitle('Market Squid landing model') + theme(legend.position = "bottom")  +  xlim(0, 10) 
```
...
plot(conditional_effects(fit_qMSQD, "PSDN_SDM_60_dOpen", surface=TRUE), ask = FALSE)
```  
...
```{r MSQD_effect_total_msqd, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Conditional effect of probability of occurrence of Northern Anchovy on Squid landings."}
...
plot(conditional_effects(
  fit_qMSQD_v7, "PSDN_SDM_dOpen", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
```

```{r by_port_msqd_sdm_msqd}
plot(conditional_effects(
  fit_qMSQD_v5, "MSQD_SDM", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
```

```{r by_port_msqd_dclose}
plot(conditional_effects(
  fit_qMSQD_120, "dClose", conditions = conditions, 
...
plot(conditional_effects(
  fit_qMSQD_v5, "PSDN_SDM_dOpen:MSQD_SDM", surface=TRUE, conditions = conditions_dClose, 
  re_formula = NULL), ncol = 4, points = FALSE)
```
...
plot(conditional_effects(fit_qMSQD_v7, "PSDN_SDM_dOpen:MSQD_SDM", surface=TRUE), ask = FALSE)
```   
