...
<< WORK ON THIS >>
For the multispecies participation model, we follow a similar methodology than @richerson201. We estimate a discrete choice model for probability that a vessel participate in a particular fishery during a specific month. First, we identify vessels that participate in at least two fishery. Then, we...
We expect that using monthly data allows us to observe seasonality in fishers´behavior, and to reduce the risk to not loosing general behavior when we dissagregate data in a fine scale. 
...
This approach allows us to understand hoe a closure or abrupt weather/abundance changes restructure fisher's portfolios. Variables that can explain fishery participation on a particular fishery are **HHI and LI?** (Think about this) (Peña et al have some ideas how to use expected catch and cost. It could be the month average...)
