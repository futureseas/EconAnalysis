...
```{r psdn_data_all, message=FALSE, warning=FALSE, include=FALSE, results='asis'}

est_data_psnd_all <- est_data %>%
  dplyr::select(port_num, Port, PSDN_SDM, MSQD_SDM, PSDN_Landings, Landing_year, ACL_PSDN) %>%
  dplyr::mutate(PSDN_Landings = coalesce(PSDN_Landings, 0)) %>% 
...
# Select ports that at least they have landing PSDN once, and drop N/A #
Tot.landings.ports <- as.data.frame(cbind(rowsum(est_data_psnd_all$PSDN_Landings, est_data_psnd_all$Port, na.rm = TRUE)))
...
### NOTE: If I use drop_na, some ports without MSQD landings are excluded. ###
est_data_psnd_all <- est_data_psnd_all %>%
  merge(Tot.landings.ports, by.x = "Port", by.y = "Port", all.x = TRUE, all.y = FALSE) %>%
  filter(dTotalPSDN==1) %>%
  drop_na() %>%
  mutate(dClose = ifelse(ACL_PSDN == 0, 1, 0))
  
# Create matrices and vectors to run STAN model #
year_id <- as.vector(cbind(as.numeric(factor(est_data_psnd_all$Landing_year))));
est_data_psnd_all$port_ID <- udpipe::unique_identifier(est_data_psnd_all, fields = "port_num", start_from = 1) 

portID_names_all <- est_data_psnd_all %>%
  dplyr::select(Port, port_ID) %>%
  unique()
```
...
fit_qPSDN_gamma_close <- brm(
  bf(PSDN_Landings ~ s(PSDN_SDM, MSQD_SDM, by = port_ID) + s(ACL_PSDN) + dClose + (1 | port_ID),
                hu ~ s(PSDN_SDM, MSQD_SDM, by = port_ID) + s(ACL_PSDN) + dClose + (1 | port_ID)),
                          data = est_data_psnd_all,
                          family = hurdle_gamma(), 
                          control = list(adapt_delta = 0.999, max_treedepth = 20),
                          chains = 4, cores = 4)
...
### Compare models 
```{r compare_dist, eval=FALSE, fig.cap="Graphical posterior predictive checks. Lognormal v/s Gamma Distribution", message=FALSE, warning=FALSE, include=FALSE}
pp_logn <- pp_check(fit_qPSDN_lognormal) + ggtitle('Lognormal distribution') + theme(legend.position = "none")
pp_gamm <- pp_check(fit_qPSDN_gamma) + ggtitle('Gamma distribution') + theme(legend.position = "bottom")

pp_gamm <- pp_check(fit_qPSDN_gamma_close) + ggtitle('Gamma distribution') + theme(legend.position = "bottom")
...
```{r y_rep_gamma_close, message=FALSE, warning=FALSE, fig.cap = "Graphical posterior predictive checks. (Gamma distribution)"}
pp_check(fit_qPSDN_gamma_close) + ggtitle('Gamma distribution (including closure)') + theme(legend.position = "bottom")
```

```{r yrep_log, message=FALSE, warning=FALSE, fig.cap = "Graphical posterior predictive checks. (Lognormal distribution)"}
pp_check(fit_qPSDN_lognormal) + ggtitle('Lognormal distribution') + theme(legend.position = "bottom")
```


```{r model-comp, message=FALSE, warning=FALSE, results='asis'}
fit_qPSDN_gamma <- add_criterion(fit_qPSDN_gamma, "loo")
fit_qPSDN_lognormal <- add_criterion(fit_qPSDN_lognormal, "loo")

comp <- loo_compare(loo(fit_qPSDN_gamma), loo(fit_qPSDN_lognormal)) %>%
  as.data.frame() %>%
...
mw1 <- model_weights(fit_qPSDN_gamma, fit_qPSDN_lognormal, weights = "loo")[rownames(comp)]
...
```{r PSDN_effect_total, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Conditional effect of SDMs on Pacific Sardine landings."}

# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qPSDN_gamma, "PSDN_SDM", surface=TRUE), ask = FALSE)
```  

```{r MSQD_effect_total, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Conditional effect of SDMs on Pacific Sardine landings."}

# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qPSDN_gamma, "MSQD_SDM", surface=TRUE), ask = FALSE)
```  

```{r ACL_effect, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Conditional effect of SDMs on Pacific Sardine landings."}
...
```{r int_effect_total, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Conditional effect of SDMs on Pacific Sardine landings."}
...
est_data_msqd$dACL_MSQD <- factor(est_data_msqd$dACL_MSQD)
str(est_data_msqd$dACL_MSQD)

fit_qMSQD_lognormal <- brm(
  bf(MSQD_Landings ~ s(PSDN_SDM, MSQD_SDM, by = port_ID) + dACL_MSQD + (1 | port_ID), 
                hu ~ s(PSDN_SDM, MSQD_SDM, by = port_ID) + dACL_MSQD + (1 | port_ID)),
                           data = est_data_msqd,
                           family = hurdle_lognormal(), 
                           control = list(adapt_delta = 0.999, max_treedepth = 20),
                           chains = 4, cores = 4)
...
fit_qMSQD_gamma_all_close <- brm(
  bf(MSQD_Landings ~ s(PSDN_SDM, MSQD_SDM, by = port_ID) + dACL_MSQD + dClose + (1 | port_ID),
                hu ~ s(PSDN_SDM, MSQD_SDM, by = port_ID) + dACL_MSQD + dClose + (1 | port_ID)),
                       data = est_data_msqd_all,
                       family = hurdle_gamma(), 
                       control = list(adapt_delta = 0.999, max_treedepth = 20),
                       chains = 4, cores = 4)
...
### Compare models 
```{r compare_dist_msqd, eval=FALSE, fig.cap="Graphical posterior predictive checks. Lognormal v/s Gamma Distribution for MSQD landings model", message=FALSE, warning=FALSE, include=FALSE}
pp_logn <- pp_check(fit_qMSQD_lognormal) + ggtitle('Lognormal distribution') + theme(legend.position = "none")
pp_gamm <- pp_check(fit_qMSQD_gamma) + ggtitle('Gamma distribution') + theme(legend.position = "bottom")
...
```{r yrep_log_msqd, message=FALSE, warning=FALSE, fig.cap = "Graphical posterior predictive checks for Market Squid landinsg model. (Lognormal distribution)"}
pp_check(fit_qMSQD_lognormal) + ggtitle('Lognormal distribution') + theme(legend.position = "bottom")
```

```{r model-comp-msqd, message=FALSE, warning=FALSE, results='asis'}
fit_qMSQD_gamma_gamma <- add_criterion(fit_qMSQD_gamma, "loo")
fit_qMSQD_gamma_lognormal <- add_criterion(fit_qMSQD_lognormal, "loo")

comp <- loo_compare(loo(fit_qMSQD_gamma), loo(fit_qMSQD_lognormal)) %>%
  as.data.frame() %>%
  select(elpd_loo, elpd_diff, se_diff, p_loo, looic) %>%
  dplyr::rename(
    "ELPD-Diff" = elpd_diff, "SE-Diff" = se_diff, 
...
mw1 <- model_weights(fit_qMSQD_gamma, fit_qMSQD_lognormal, weights = "loo")[rownames(comp)]
...
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qMSQD_gamma, "PSDN_SDM", surface=TRUE), ask = FALSE)
```  
...
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qMSQD_gamma, "MSQD_SDM", surface=TRUE), ask = FALSE)
```  
```{r ACL_effect_msqd, echo=FALSE, fig.cap="Effect ACL on MSQD", message=TRUE, warning=TRUE}
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qMSQD_gamma, "dACL_MSQD"), ask = FALSE)
```  
...
plot(conditional_effects(
  fit_qMSQD_gamma, "PSDN_SDM", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
```
...
plot(conditional_effects(
  fit_qMSQD_gamma, "MSQD_SDM", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
...
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qMSQD_gamma, "PSDN_SDM:MSQD_SDM", surface=TRUE), ask = FALSE)
...
plot(conditional_effects(
  fit_qMSQD_gamma, "PSDN_SDM:MSQD_SDM", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
```
...
```{r int_effect_sep, fig.cap = "SDMs effect Landings and Hurdle."}
...
```{r int_effect_sep_msqd, fig.cap = "SDMs effect Landings and Hurdle."}
