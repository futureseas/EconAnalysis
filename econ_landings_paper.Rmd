...
Landings in the CPS fishery have been mainly dominated by Pacific sardine and market squid during the period 1981-2014, before the closure of the Pacific sardine fishery, as is shown in panel (a) in Figure \ref{fig:avg_lan_rev}. As prices are similar between species, except for market squid as is shown in panel (b), low average annual landings observed for Pacific herring and Northern anchovy could be related with geography and regulatory constraint that restrict vessels to exploit them. In terms of revenue, we can observe in panel (c) that market squid have the largest revenue, while Pacific sardine is not as relevant under this comparison due to the low prices received by fishers.
...
Figure \ref{fig:ts_landings} shows the evolution of landings, prices and vessel participation by species during the period 1981-2020. We can observe in panel (a) that real prices for market squid have been increasing in general, and also relative to other species. Incentives should be high for landing market squid. However, in panel (b) we observe a sharp decline on market squid landings in the last decade. Prices are thought to be determined in the world market, as the California current landings for forage species corresponde to a small share of the total landings in the world. A potential hypothesis for the observed decline in market squid landings could be that the lower abundance and the consequently closure of the Pacific sardine fishery also affect the decision to participate of vessels in the market squid fishery as become more risky when an species is not available for income diversification. The decline of vessel participation in the market squid fishery observed in panel (c) could be suggesting this hypothesis. Another hypothesis is that other species, not listed in our list of CPS species, have higher prices compared to market squid, encouraging market squid vessel to switch gear and targets. ***<<NOTE: WE HAVE TO THINK IN OTHER SPECIES THAT MARKET SQUID VESSEL CAN SWITH >>***

```{r ts_landings, echo=FALSE, fig.cap="Yearly total landing, average price and total number of vessel by CPS species (1981 - 2020).\\label{fig:ts_landings}", message=FALSE, warning=FALSE}
