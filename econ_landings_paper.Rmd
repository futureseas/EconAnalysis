...
$$
[\theta_p | q_{it}] \varpropto f\left(q_{it} | \theta_p\right) \times [\theta_p]
$$
where $q_{it}$ is the observed landings of the corresponding species by vessel $i \in (1,\ldots,L)$ at year $t$, $L$ is the total number of vessels, and $\theta_i$ are the parameters (i.e., random-coefficients) to be estimated by ports $p \in (1,\ldots,P)$, where $P$ is the total number of ports. The latter gives the name of hierarchical to our model.^[For more details about Hierarchical models, see \cite{hobbs2015}.] 
The distribution $f\left(q_{it} | \theta_i\right)$ can be rewritten as:
$$ f\left(q_{it} | \theta_p \right) = 
