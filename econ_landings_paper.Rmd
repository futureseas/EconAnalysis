...
<!-- - Graficar número de barcos que desembarcan especies de CPS durante cada mes.  -->
...
sdm.by.species <- PacFIN.month.CPS %>%
  dplyr::select(LANDING_YEAR, LANDING_MONTH, PORT_NAME, PSDN_SDM_60, MSQD_SDM_90, NANC_SDM_20, 
                LANDED_WEIGHT_MTONS.sum, PACFIN_SPECIES_CODE) %>% filter(LANDING_YEAR >= 1998 & LANDING_YEAR <= 2019) %>%
  group_by(LANDING_YEAR, LANDING_MONTH, PORT_NAME, PACFIN_SPECIES_CODE) %>% 
  summarize(PSDN_SDM_60 = mean(PSDN_SDM_60, na.rm = TRUE), 
            ...
            LANDED_WEIGHT_MTONS = sum(LANDED_WEIGHT_MTONS.sum)) %>%
