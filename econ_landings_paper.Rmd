...
```{r n_species_by_gear, echo=FALSE, echo=FALSE, results='asis'}
...
PacFIN.CPS <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% 
  unique() %>%  mutate(Species = 1) %>%
  group_by(VESSEL_NUM, PACFIN_GEAR_CODE) %>% 
  summarise(N_Species_landed = sum(Species)) 
  summary(PacFIN.CPS$N_Species_landed)
  
table <- xtabs(N_Species_landed ~ PACFIN_GEAR_CODE, aggregate(N_Species_landed ~ PACFIN_GEAR_CODE, PacFIN.CPS, mean))
table
# rownames(table) = c("Crab & Lobster Pot", "Danish/Scottish Seine", "Dip Net", "Diving Gear" , "Flatfish Trawl", "Fish Pot", "Selective Flatfish Trawl", "Ground Fish Trawl < 8 inches", "Ground Fish Trawl (Otter)", "Gill Net", "Longline", "Midwater Trawl", "Other Net Gear", "Pole", "Roller Trawl",  "Seine", "Shrimp Trawl") 

print(xtable(table, caption = 'Average number of Species landed by a vessel by gear used.', type = "latex"), comment=FALSE,  caption.placement = "top")

```



```{r n_species_CPS_by_gear, echo=FALSE, echo=FALSE, results='asis'}

# Create dummy of whether a vessel harvest MSQD, NANC OR PSDN 
PacFIN.CPS <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("PSDN", "MSQD", "NANC", "PHRG")) %>%  mutate(CPS_species = 1) %>%
  group_by(VESSEL_NUM, PACFIN_GEAR_CODE) %>% 
  summarise(N_CPS_landed = sum(CPS_species)) 
  summary(PacFIN.CPS$N_CPS_landed)
  
table <- xtabs(N_CPS_landed ~ PACFIN_GEAR_CODE, aggregate(N_CPS_landed ~ PACFIN_GEAR_CODE, PacFIN.CPS, mean))
table
# rownames(table) = c("Crab & Lobster Pot", "Danish/Scottish Seine", "Dip Net", "Diving Gear" , "Flatfish Trawl", "Fish Pot", "Selective Flatfish Trawl", "Ground Fish Trawl < 8 inches", "Ground Fish Trawl (Otter)", "Gill Net", "Longline", "Midwater Trawl", "Other Net Gear", "Pole", "Roller Trawl",  "Seine", "Shrimp Trawl") 

print(xtable(table, caption = 'Average number of CPS species landed by a vessel by gear used.', type = "latex"), comment=FALSE,  caption.placement = "top")
