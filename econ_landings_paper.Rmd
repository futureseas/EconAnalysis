...
dataset_psdn <- dataset %>% 
  dplyr::select(PSDN_SDM_60, PSDN_Landings, PSDN_Price, ACL_PSDN,
                MSQD_SDM_90, PORT_ID, PACFIN_PORT_CODE, PORT_AREA_CODE, LANDING_YEAR, LANDING_MONTH) %>% 
  ...
  filter(dTotalPSDN==1) %>% filter(dTotalMSQD==1) %>% drop_na()
...
dataset_psdn$port_ID <- as.factor(
  udpipe::unique_identifier(dataset_psdn, fields = "PORT_AREA_CODE", start_from = 1))
