...
Before continuing our analysis, we have to verify whether there is switching behavior between CPS species. Moreover, 
categorizing vessel in different groups allows us to estimate differentiated coefficients to capture different responses to an explanatory variable. We conduct a cluster analysis to categorize the different portfolios fishers manage during a year. For this, we conduct our cluster analysis using a hierarchical clustering method. This method has been used before in fisheries to define *metiers* in discrete choice models [@vermard2008]. This approach does not require defining the number of clusters chosen within the algorithm. There are different methods to compute the distance between two clusters and conduct the algorithm. We use Ward's method in our study, similar to [@vermard2008]. The goal of this method is to reduce the within-cluster variance. The algorithm starts with all observation in a cluster and then merge in each step the two cluster with the minimum distance between cluster. The analysis includes commercial vessels that have participated at least in one CPS during 2000-2014. 

Table \ref{Table:cluster_all} present the results for the clustering analysis. Many vessels belong to a cluster that describes a portfolio considered as single-species. For instance, a vessel belonging to Cluster 1 in average 99.42% of the total landings correspond to market squid. Thus, these vessels only focus on this species—vessels in cluster 2 only focus on Pacific sardine, and vessels in cluster 7 only focus on Pacific mackerel. In contrast, clusters 3, 4, and 5 show substitution between market squid and Pacific sardine, while cluster 6 shows substitution between Northern anchovy and Pacific sardine. In the following subsections, we study gear used and port landed by vessel depending on the species landed and the vessel's cluster. 
...
PacFIN.spread <- PacFIN.month.CPS %>% filter(LANDING_YEAR >=2000) %>%  filter(LANDING_YEAR < 2015) %>%
  ...
  PacFIN.spread.perc <- PacFIN.spread %>% select("CMCK", "MSQD", "NANC", "PSDN", "JMCK", "UMCK") %>%
    mutate(Total = CMCK + MSQD + NANC + PSDN + JMCK + UMCK) %>%
    filter(Total > 0) %>%
...
rm(PacFIN.spread.perc, PacFIN.spread , clusterCut, clusters, PacFIN.cluster, PacFIN.clusters)
...
The ability to switch using seine as gear is confirmed when we analyze the most imnportant gears used by different clusters in terms of landings (see Table \ref{Table:cluster_gear}). Total landings of cluster where substitution is observable comes mainly from catches using seine, and in some cases from other net gears.  <!-- In Cluster 5, the cluster where we observe substitution, 88\% of the cluster's total landings correspond to catches using seine as a gear. Only 7\% and 3\% of the cluster's total landings correspond to catches using other net gear and midwater trawl, respectively. -->
...
On average, vessels participating in the CPS fishery land their catch in 7.74 ports, with a standard deviation of 4.79. If we only consider landings of CPS species, vessels land in 5.36 ports on average, with a standard deviation of 3.65. When we disaggregate this information by species, vessels that participate in the corresponding fishery land on average market squid in 5.37 ports, Pacific sardine in 3.96 ports, Northern anchovy in 3.17 ports, and Pacific mackerel in 2.24 ports. 

Using the results from the cluster analysis, we can identify how many ports, on average, a vessel that belongs to a cluster lands. On average, vessels belonging to Cluster 1 land in 4 ports, Cluster 2 land in 2.9 ports, Cluster 3 land in 6.97 ports, Cluster 4 land in 7.33 ports, Cluster 5 land in 8.64 ports, Cluster 6 land in 3.12, and Cluster 7 land in  2.08. Thus, vessels with a multiple species portfolio (cluster 3, 4, 5 and 6) on average land in more ports than vessels that only focus on one species, specially when they switch mainly between market squid and Pacific sardine (cluster 3, 4 and 5). Cluster that show substitutions mostly land in Southern California (see Table \ref{Table:cluster_port}).
...
For our empirical model, we focus on vessels that **actively particiáte in the CPS fishery and belong to Cluster 5**, in which we observe substitution, and on vessels that land in the following area ports: Los Angeles (e.g., Terminal Island and San Pedro), Santa Barbara (e.g., Port Hueneme and Ventura), and Monterrey (e.g., Monterrey and Moss Landing). **This data subset allows us only to incorporate ports and vessels with substitution capacity**. Our data set contains variables measured at the port and vessel levels for the CPS fishery in the U.S. West Coast. In our landing model, our outcome variable is the vessel's landings in a month of a specific species. We expect that using quarterly data allows us to observe seasonality in fishers' behavior and reduce the risk of losing general behavior when we disaggregate data on a finer scale. <!-- These two different outcome variables would allow us to study the degree of flexibility that vessel has compared to ports regarding species substitution and catch composition. If vessels have firm contracts with processors associated with a port, we should observe that the substitution of vessels and port is similar.  --> We obtained daily vessel level panel data on landing by port areas from 1981 to 2020 upon request from [PacFIN](http://pacfin.psmfc.org/). 
