...
PacFIN_yearly <- read.csv(file = here::here("Data", "PacFin_yearly.csv"))
PacFIN_month <- read.csv(file = here::here("Data", "PacFin_monthly.csv"))
sapply(PacFIN_month, class) 

# names(PacFIN_yearly)[names(PacFIN_yearly) == "LANDING_YEAR"              ] <- "Year"
# names(PacFIN_yearly)[names(PacFIN_yearly) == "PACFIN_SPECIES_CODE"       ] <- "Species_code"
# names(PacFIN_yearly)[names(PacFIN_yearly) == "PACFIN_SPECIES_COMMON_NAME"] <- "Species_name"
# names(PacFIN_yearly)[names(PacFIN_yearly) == "LANDED_WEIGHT_MTONS"       ] <- "Landings"
# names(PacFIN_yearly)[names(PacFIN_yearly) == "EXVESSEL_REVENUE"          ] <- "Revenue"
# names(PacFIN_yearly)[names(PacFIN_yearly) == "LANDED_WEIGHT_PPP"         ] <- "Price"
# names(PacFIN_yearly)[names(PacFIN_yearly) == "AGENCY_CODE"               ] <- "State"
# names(PacFIN_yearly)[names(PacFIN_yearly) == "MANAGEMENT_GROUP_CODE"     ] <- "Management_group"
...
Substitution between species is likely to be related with the gear that a vessel or fisher have already invest for their fishing activities. However, a vessel is not restricted to an specific gear. They can switch between different alternatives. In average a vessel use 2.96 gears for their fishing activities, with an standard deviation of 1.96.

```{r n_gear_used_vessels, echo=FALSE, echo=FALSE, results='asis'}
...
Table \ref{table:gears} show the total landed wight in tons during the period 2000-2020 for different CPS species by gear. Seine is the most important gear used between CPS fishery, however, is not common for Pacific Herring.


```{r total_hist_landing_by_gear, echo=FALSE, echo=FALSE, results='asis'}

PacFIN.CPS <- PacFIN_month %>% 
  filter( PACFIN_SPECIES_CODE  %in% c("MSQD", "PSDN", "NANC", "PHRG")) %>% 
  filter( PACFIN_GEAR_CODE  %in% c("DPN", "ONT", "SEN", "GLN", "GLN", "MDT")) %>%
  filter( LANDING_YEAR >= 2010 )

table <- round(xtabs(LANDED_WEIGHT_LBS.sum ~ PACFIN_SPECIES_CODE + PACFIN_GEAR_CODE, data = PacFIN.CPS )/2204.6, 0 )
rownames(table) = c("Market squid", "Northern Anchovy", "Pacific Herring", "Pacific Sardine") 
colnames(table) = c("Dip Net", "Gill Net", "Midwater Trawl", "Other Net Gear", "Seine") 
print(xtable(table, caption = 'Landings of CPS species by gear used. \\label{table:gears}', type = "latex"), comment=FALSE,  caption.placement = "top")
...
```{r lenght_by_species_gear, echo=FALSE, echo=FALSE, results='asis'}
...
table <- xtabs(VESSEL_LENGTH.mean ~ PACFIN_SPECIES_CODE + PACFIN_GEAR_CODE,
               aggregate(VESSEL_LENGTH.mean ~ PACFIN_SPECIES_CODE + PACFIN_GEAR_CODE, PacFIN.CPS, mean))
table
rownames(table) = c("Market squid", "Northern Anchovy", "Pacific Herring", "Pacific Sardine") 
colnames(table) = c("Dip Net", "Gill Net", "Midwater Trawl", "Other Net Gear", "Seine") 
print(xtable(table, caption = 'Vessel lenght used in the CPS fishery by gear used', type = "latex"), comment=FALSE,  caption.placement = "top")

```


```{r fishing_days, echo=FALSE, echo=FALSE, results='asis'}
...
table <- xtabs(NUM_OF_DAYS_FISHED.mean ~ PACFIN_SPECIES_CODE + PACFIN_GEAR_CODE,
               aggregate(NUM_OF_DAYS_FISHED.mean ~ PACFIN_SPECIES_CODE + PACFIN_GEAR_CODE, PacFIN.CPS, mean))
table
rownames(table) = c("Northern Anchovy", "Pacific Herring", "Pacific Sardine") 
colnames(table) = c("Dip Net", "Midwater Trawl", "Other Net Gear", "Seine") 
...
<!-- 2) How many species a vessel harvest from the CPS -->



```{r landing_rev_by_gear, echo=FALSE, echo=FALSE, results='asis'}

PacFIN.CPS <- PacFIN_month %>% filter( LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE) %>% unique()

# Create dummy of whether a vessel harvest MSQD, NANC OR PSDN 
PacFIN.CPS.3 <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("PSDN")) %>% select(VESSEL_NUM) %>% unique() %>%
  mutate(CPS_landing = 1) 

PacFIN.CPS.n_species <- PacFIN.CPS %>% merge(PacFIN.CPS.3, by = "VESSEL_NUM", all.x = TRUE) %>%
  filter(CPS_landing == 1) %>% unique() %>% group_by(VESSEL_NUM) %>% 
  summarise(N_species_landed = sum(CPS_landing)) 

summary(PacFIN.CPS.n_species$N_species_landed)

# By gear?

# Create dummy of whether a vessel harvest MSQD, NANC OR PSDN 
PacFIN.CPS.3 <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% 
  unique() %>% filter(PACFIN_SPECIES_CODE  %in% c("PSDN")) %>% select(VESSEL_NUM, PACFIN_GEAR_CODE) %>% unique() %>%
  mutate(CPS_landing = 1) 

### Does a vessel can change between gear? Easy between nets???

PacFIN.CPS <- PacFIN_month %>% filter( LANDING_YEAR >= 2010) %>% 
  select(VESSEL_NUM, PACFIN_SPECIES_CODE, PACFIN_GEAR_CODE) %>% unique()
PacFIN.CPS.n_species <- PacFIN.CPS %>% inner_join(PacFIN.CPS.3, by = c("VESSEL_NUM", "PACFIN_GEAR_CODE")) %>%
  filter(CPS_landing == 1) %>% unique() %>% group_by(VESSEL_NUM, PACFIN_GEAR_CODE) %>% 
  summarise(N_species_landed = sum(CPS_landing)) 

summary(PacFIN.CPS.n_species$N_species_landed)

table <- xtabs(N_species_landed ~ PACFIN_GEAR_CODE, aggregate(N_species_landed ~ PACFIN_GEAR_CODE, PacFIN.CPS.n_species, mean))
table
rownames(table) = c("Market squid", "Northern Anchovy", "Pacific Herring", "Pacific Sardine") 
colnames(table) = c("Dip Net", "Gill Net", "Midwater Trawl", "Other Net Gear", "Seine") 
print(xtable(table, caption = 'Gear used in the CPS fishery.', type = "latex"), comment=FALSE,  caption.placement = "top")

```


```{r n_gear_used, echo=FALSE, echo=FALSE, results='asis'}

# Create dummy of whether a vessel harvest MSQD, NANC OR PSDN 
PacFIN.CPS.3 <- PacFIN_month %>% filter(LANDING_YEAR >= 2010) %>% select(VESSEL_NUM, PACFIN_GEAR_CODE) %>% 
  unique() 

PacFIN.CPS.n_gear <- PacFIN.CPS.3 %>% mutate(N_gear = 1) %>% group_by(VESSEL_NUM) %>%
  summarise(N_gear= sum(N_gear)) 

summary(PacFIN.CPS.n_gear$N_gear)


### If I have use SEINE, how many other gear I use?


PacFIN.CPS <- PacFIN_month %>% filter( LANDING_YEAR >= 2010) %>%  select(VESSEL_NUM, PACFIN_GEAR_CODE) %>% unique()

PacFIN.CPS.seine <- PacFIN_month %>% filter( LANDING_YEAR >= 2010) %>% 
  select(VESSEL_NUM, PACFIN_GEAR_CODE) %>% filter(PACFIN_GEAR_CODE == "SEN") %>% mutate(D_Seine = 1) %>% select(VESSEL_NUM, D_Seine)  %>% unique() 

PacFIN.CPS.n_gear <- PacFIN.CPS %>% inner_join(PacFIN.CPS.seine, by = c("VESSEL_NUM")) %>%
  filter(D_Seine == 1) %>% unique() %>% group_by(VESSEL_NUM) %>% 
  summarise(N_gear= sum(D_Seine)) 
...
<!-- 2) How many ports vessel land their harvest -->
...
landings <- summaryBy(Landings ~ Species_code + Landing_year + Management_group, FUN=sumfun, data=PacFIN_yearly) %>%
  filter(Management_group == "CPEL") %>%
  filter(Species_code != "PBNT" & Species_code != "CMCK" & Species_code != "PHRG" &   
...
g1 <- ggplot(landings, aes(Species_code, Landings.sum.mean)) +
  ...
  scale_x_discrete(labels=c("MSQD" = "Market\nSquid", "NANC" = "Northern\nAnchovy", "PSDN" = "Pacific\nSardine"))


revenue <- summaryBy(Revenue ~ Species_code + Landing_year + Management_group, FUN=sumfun, data=PacFIN_yearly) %>%
  filter(Management_group == "CPEL") %>%
  filter(Species_code != "PBNT" & Species_code != "CMCK" & Species_code != "PHRG" & 
           Species_code != "JMCK" & Species_code != "RHRG" & Species_code != "UMCK") 
...
```{r ts_landings, echo=FALSE, fig.cap="Total annual landing by CPS species.\\label{fig:ts_landings}", message=FALSE, warning=FALSE}
collapse <- PacFIN_yearly %>% filter(Landing_year <= 2020) %>%
  filter(Management_group == "CPEL") %>%
  filter(Species_code != "PBNT" & Species_code != "CMCK" & Species_code != "JMCK" & Species_code != "PHRG"
         & Species_code != "RHRG" & Species_code != "UMCK")

collapse <- summaryBy(Landings + Price + N_vessels ~ Species_name  + Landing_year, FUN=sum_mean_fun, data=collapse) %>%
  dplyr::select(Landing_year, Price.mean, Landings.sum, N_vessels.mean, Species_name) 


g1 <- ggplot(collapse, aes(x = Landing_year, y = Landings.sum, group= Species_name, colour =  Species_name)) + 
  geom_line(size=1) + scale_x_continuous(name = element_blank()) +
  ...
  scale_color_brewer(palette="Set2", labels=c("MARKET SQUID" = "Market Squid", 
                                              "NORTHERN ANCHOVY" = "Northern Anchovy", 
                                              "PACIFIC SARDINE" = "Pacific Sardine"))
  
g2 <- ggplot(collapse, aes(x = Landing_year, y = Price.mean, group= Species_name, colour =  Species_name)) + 
  geom_line(size=1) + scale_x_continuous(name = "Year") +
  scale_y_continuous(name = "Price ($/Lbs)")  + 
   scale_color_brewer(palette="Set2") +  theme(legend.position = "none", plot.title = element_text(size=9, face="bold.italic"), 
        axis.text = element_text(size = 7), axis.title = element_text(size = 8)) + 
  ggtitle("(b) Prices")

g3 <- ggplot(collapse, aes(x = Landing_year, y = N_vessels.mean, group= Species_name, colour =  Species_name)) + 
  geom_line(size=1) + scale_x_continuous(name = element_blank()) +
