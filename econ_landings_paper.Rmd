...
est_data_psnd <- est_data %>%
  dplyr::select(port_num, Port, PSDN_SDM, MSQD_SDM_90, PSDN_Landings, Landing_year, ACL_PSDN) %>%
...
fit_qPSDN <- brm(
  bf(PSDN_Landings ~ PSDN_SDM + MSQD_SDM + s(ACL_PSDN) + (1 + PSDN_SDM + MSQD_SDM | port_ID),
                hu ~ PSDN_SDM + MSQD_SDM + s(ACL_PSDN) + (1 + PSDN_SDM + MSQD_SDM | port_ID)),
                          data = est_data_psnd,
                          family = hurdle_gamma(), 
...
pp_check(fit_qPSDN_v2) + ggtitle('Pacific Sardine model') + theme(legend.position = "bottom")
```
...
plot(conditional_effects(fit_qPSDN_v2, "PSDN_SDM", surface=TRUE), ask = FALSE)
```  

```{r MSQD_effect_total, echo=FALSE, message=FALSE, warning=FALSE}

# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qPSDN_v2, "MSQD_SDM", surface=TRUE), ask = FALSE)
```  

```{r ACL_effect, echo=FALSE, message=FALSE, warning=FALSE}
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qPSDN_v2, "ACL_PSDN", surface=TRUE), ask = FALSE)
```  
...
plot(conditional_effects(
  fit_qPSDN_v2, "PSDN_SDM", surface=TRUE, conditions = conditions, 
...
```{r by_port_msqd_sdm}
plot(conditional_effects(
  fit_qPSDN_v2, "MSQD_SDM", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
...
```{r by_port_acl_psdn}
plot(conditional_effects(
  fit_qPSDN_v2, "ACL_PSDN", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
...
```{r int_effect_total, echo=FALSE, message=FALSE, warning=FALSE}

# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qPSDN_v2, "PSDN_SDM:MSQD_SDM", surface=TRUE), ask = FALSE)
...
plot(conditional_effects(
  fit_qPSDN_v2, "PSDN_SDM:MSQD_SDM", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
```
...
est_data_msqd_all <- est_data %>%
  dplyr::select(port_num, Port, PSDN_SDM, MSQD_SDM_60, MSQD_SDM_90, MSQD_SDM_120, MSQD_Landings, Landing_year, ACL_MSQD) %>%
  ...
  dplyr::mutate(PSDN_SDM_dOpen = PSDN_SDM * dOpen)
...
fit_qMSQD <- brm(
 bf(MSQD_Landings ~ MSQD_SDM_90 + PSDN_SDM_dOpen + dClose + (1 + MSQD_SDM_60 + PSDN_SDM_dOpen + dClose | port_ID),
               hu ~ MSQD_SDM_90 + PSDN_SDM_dOpen + dClose + (1 + MSQD_SDM_60 + PSDN_SDM_dOpen + dClose | port_ID)),
...
comp <- loo_compare(loo(fit_qMSQD_v5), loo(fit_qMSQD_v7)) %>%
  as.data.frame() %>%
...
mw1 <- model_weights(fit_qMSQD_v5, fit_qMSQD_v7, weights = "loo")[rownames(comp)]
...
pp_check(fit_qMSQD_v5) + ggtitle('Gamma distribution + dClose') + theme(legend.position = "bottom")  +  xlim(0, 10) 
```
...
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qMSQD_v5, "PSDN_SDM_dOpen", surface=TRUE), ask = FALSE)
```  
...
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qMSQD_v5, "MSQD_SDM", surface=TRUE), ask = FALSE)
```  

```{r ACL_effect_msqd, echo=FALSE, fig.cap="Effect ACL on MSQD", message=TRUE, warning=TRUE}
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qMSQD_120, "dClose", dpar="mu"), ask = FALSE)
```  
