...
# Transform ID data to numeric, obtain unique SDMs and ACL, and change NaN to NA.
PacFIN.month.dataset <- PacFIN.month.dataset %>%
        ...
        mutate(MSQD_SPAWN_SDM_90 = rowMeans(cbind(PSDN_MSQD_SPAWN_SDM_90, MSQD_MSQD_SPAWN_SDM_90), na.rm = T)) %>%
...
dataset = subset(PacFIN.month.dataset, select = -c(PSDN_PSDN_SDM_60, MSQD_PSDN_SDM_60, 
                  PSDN_MSQD_SDM_90, MSQD_MSQD_SDM_90, 
                  PSDN_MSQD_SPAWN_SDM_90, MSQD_MSQD_SPAWN_SDM_90,
                  PSDN_ACL_PSDN, MSQD_ACL_PSDN))

rm(PacFIN.month.dataset, port_area, selected.ports)
...
Our Bayesian framework incorporates a hurdle model to model the zeros included in our landing data. We observe a zero when no landings occur in a quarter for a particular vessel (we transformed landings observations with "N/A" to zero). We assume that these zeros mean no incentives to participate in the related fishery for the vessel during a particular quarter. Therefore, the hurdle part of the model is our participation model. The hurdle model is similar to the methodology used by @richerson2017. They estimate a discrete choice model for the probability of participating in a fishery. We expand their approach, merging this model with the landing model developed by @smithpotential in a single model. <!-- According @richerson2017, fishers participation decision should depend, in general terms, on profitability, species distribution, and regulations. --> Our model allows us to estimate the total effect of, for example, a closure on landings, considering any effect that the explanatory variables have on participation before considering the level of landings.   
...
dataset <- dataset %>% 
  ...
  # filter(nSpecies==2) %>% 
  filter(PORT_AREA_CODE %in% c('LAA', 'CLO', 'SBA', 'MNA')) %>% filter(cluster == 5) %>%
  group_by(LANDING_YEAR, VESSEL_NUM, PORT_AREA_ID) %>%
  summarise(PSDN_SDM_60 = mean(PSDN_SDM_60), MSQD_SDM_90 = mean(MSQD_SDM_90), 
            PSDN_Landings = sum(PSDN_Landings), MSQD_Landings = sum(MSQD_Landings),
            PSDN_Price = mean(PSDN_Price), MSQD_Price = mean(MSQD_Price), 
            ACL_PSDN = mean(ACL_PSDN))


desc_data <- dataset %>%
  subset(select = -c(PORT_AREA_ID, LANDING_YEAR, VESSEL_NUM))
...
dataset_psdn <- dataset %>% 
  dplyr::select(VESSEL_NUM, PSDN_SDM_60, PSDN_Landings, PSDN_Price, ACL_PSDN, MSQD_Price, MSQD_SDM_90,
                PORT_AREA_ID, LANDING_YEAR) %>%
  ...
  mutate(RelPrice = PSDN_Price / MSQD_Price) %>% 
  mutate(RelSDM = PSDN_SDM_60 / MSQD_SDM_90) %>%
  drop_na() 
## Just 2 observations are dropped
...
# Note that the random coefficient is estimated for port areas, not ports. 
# SHOULD I EXPAND TO QUARTERS??? ALSO, INCLUDE OTHER CLUSTERS???? 
### Check about substitution in supply in multispecies framwork. How is the supply curve modeled? ###


# fit_qPSDN <- brm(bf(PSDN_Landings ~ RelSDM + RelPrice + (1 | port_ID),
#                                hu ~ (1 | port_ID)),
#                     data = dataset_psdn,
#                   family = hurdle_gamma(), 
...
fit_qPSDNv2 <- brm(bf(PSDN_Landings ~ PSDN_SDM_60 + MSQD_SDM_90 + RelPrice + (1 | port_ID),
                               hu ~ (1 | port_ID)),
                    data = dataset_psdn,
                  family = hurdle_gamma(), 
                 control = list(adapt_delta = 0.999, max_treedepth = 20),
