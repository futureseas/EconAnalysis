...
```{r landing_rev_by_gear}

PacFIN.CPS <- PacFIN_month %>% 
  filter( PACFIN_SPECIES_CODE  %in% c("MSQD", "PSDN", "NANC", "PHRG")) %>% 
  filter( PACFIN_GEAR_CODE  %in% c("DPN", "ONT", "SEN", "GLN", "CLP", "GLN", "LGL", "MDT", "POL", "USP", "TRL")) %>%
  filter( LANDING_YEAR >= 2010 )


print(xtable(round(xtabs(LANDED_WEIGHT_LBS.sum ~ PACFIN_SPECIES_CODE + PACFIN_GEAR_CODE, data = PacFIN.CPS )/2204.6, 0 ), type = "latex"))
