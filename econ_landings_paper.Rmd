...
PacFIN.spread <- PacFIN.month.CPS %>% filter(LANDING_YEAR >=2000) %>%  filter(LANDING_YEAR < 2015) %>%
  ...
  PacFIN.spread.perc <- PacFIN.spread %>% select("CMCK", "MSQD", "NANC", "PBNT", 
    ...
    mutate(JMCK = JMCK / Total * 100) %>% mutate(JMCK = UMCK / Total * 100)  

# set.seed(21)
# speciesCluster <- kmeans(PacFIN.spread[, 2:5], 30)
# speciesCluster

# From this first cluster analysis, I should not include PHRG in the model. However, the number of cluster here is arbitrary. 

## Let now conduct a hierarchical clustering analysis ##
...
<!-- WORKSHOP: though there is less target switching in the northern CCS. -->
