...
g2 <- ggplot(sdm.by.species) + 
  ...
  ggtitle("(b) Market squid (Spawning biomass within 90 km radious)") +  scale_color_manual(name = "Variable: ", 
                     values = c("Landings" = "grey", "Probability of presence" = "blue"))
...
```{r cluster_analysis, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, results='asis'}
...
Hierarchical_Vessel_Groups <- read.csv("C:\\GitHub\\EconAnalysis\\Data\\Hierarchical_Vessel_Groups.csv")
  PacFIN.month.cluster <- merge(PacFIN.month, Hierarchical_Vessel_Groups,
                              by = ("VESSEL_NUM"), all.x = TRUE, all.y = FALSE)
...
Port_Groups<-read.csv("Port_Groups_1.27.2022_Check.csv")
...
PacFIN.cluster <- PacFIN.month.cluster %>% filter(LANDING_YEAR >= 2000) %>% 
  group_by(group, PACFIN_GEAR_CODE) %>% summarise(landings = sum(LANDED_WEIGHT_MTONS.sum)) %>% 
...
table <- xtabs(perc ~  PACFIN_GEAR_CODE + group, PacFIN.cluster)
# table
rownames(table) = c("Dip Net", "Other Net Gear", "Pole", "Seine", "Troll")
