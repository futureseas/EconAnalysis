...
port_area <- read.csv(file = here::here("Data", "Ports", "ports_area_codes.csv"))
  ...
  PacFIN.month.select <- PacFIN.month %>% 
    dplyr::filter(PACFIN_SPECIES_CODE %in% 
    c("PHRG", "CMCK", "JMCK", "MSQD", "NANC", "PBNT", "PSDN", "UMCK", "DCRB", "ALBC", "BTNA", "ETNA", "STNA", "UTNA"))
...
***<<< Note: I got this information from an Excel file that desiree sent me about Annual Catch Limits and Limted entry for sardine and squid, and I will start to complement with the CPS FMP document, SAFE report, and random notes that I have >>>***
...
Amendment 11 implemented in 2006 change the allocation to 20\% to Subarea A and 80\% to Subarea.

Coast-wide, the commercial fishery is currently have been closed since 2015 as the spawning stock biomass (SSB) is estimated as being below the cutoff point of 150,000 metric tons. *<<SEE SAFE REPORT FOR DETAILS ABOUT SEASONAL CLOSURES. BIG MESS AFTER 2008 UNTIL FISHERY CLOSURE>>*
...
We conduct a cluster analysis to categorize better the different portfolio that fisher manage during a year. For this we use a hierarchical clustering. This technique have been used before in fisheries. This approach do not require to define de number of cluster as is chosen within the algorithm. There are different method to compute the distance between two clusters and conduct the algorithm. We use the Ward's method in our study similar to XXX. The goal of this method is to reduce the within-cluster variance. The algorithm start with all observation in a cluster and then merge in each step the two cluster with the minimum distance between-cluster.   
...
PacFIN.spread <- PacFIN.month.CPS %>% filter(LANDING_YEAR >=2000) %>%  filter(LANDING_YEAR < 2015) %>%
  ...
  PacFIN.spread <- PacFIN.spread %>%
    mutate(Total = MSQD + NANC + PHRG + PSDN) %>%
    mutate(MSQD = MSQD / Total * 100) %>% 
    mutate(NANC = NANC / Total * 100) %>% 
    mutate(PHRG = PHRG / Total * 100) %>% 
    mutate(PSDN = PSDN / Total * 100)

set.seed(20)
speciesCluster <- kmeans(PacFIN.spread[, 2:5], 7)
speciesCluster
...
PacFIN.spread[is.na(PacFIN.spread)] <- 0
clusters <- hclust(dist(PacFIN.spread[, 2:5]), method = "ward.D")
plot(clusters, cex = 0.1, hang = -1)
clusterCut <- as.data.frame(cutree(clusters, 5))
PacFIN.spread$cluster <- clusterCut$`cutree(clusters, 5)`
  PacFIN.clusters <- PacFIN.spread %>% group_by(cluster) %>%
    summarise(mean.MSQD = mean(MSQD), mean.NANC = mean(NANC), 
              mean.PSDN = mean(PSDN), mean.PHRG = mean(PHRG), count = n())
...
library(cluster)
hc2 <- agnes(PacFIN.spread[, 2:5], method = "ward")
hc2$ac
## 0.9995197

# Make graph
library(factoextra)
fviz_cluster(list(data = cbind.data.frame(PacFIN.spread$MSQD, PacFIN.spread$PSDN), cluster = cutree(clusters, 5)))  ## from ‘factoextra’ package 

pltree(hc2, hang=-1, cex = 0.6)
rect.hclust(hc2, k = 5, border = 2:10)
