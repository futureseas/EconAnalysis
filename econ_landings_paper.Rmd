...
fit_qPSDN_gamma_v2 <- brm(
  bf(PSDN_Landings ~ s(PSDN_SDM, MSQD_SDM) + s(ACL_PSDN) + (1 + PSDN_SDM + MSQD_SDM | port_ID),
                hu ~ s(PSDN_SDM, MSQD_SDM) + s(ACL_PSDN) + (1 + PSDN_SDM + MSQD_SDM | port_ID)),
                          data = est_data_psnd,
                          family = hurdle_gamma(), 
...
LOO(fit_qPSDN_gamma, fit_qPSDN_gamma_v2)
...
pp_check(fit_qPSDN_gamma_v2) + ggtitle('Gamma distribution') + theme(legend.position = "bottom")
```
...
```{r PSDN_effect_total, echo=FALSE, fig.cap=, message=FALSE, warning=FALSE}

# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qPSDN_gamma_v2, "PSDN_SDM", surface=TRUE), ask = FALSE)
```  

```{r MSQD_effect_total, echo=FALSE, fig.cap=, message=FALSE, warning=FALSE}

# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qPSDN_gamma_v2, "MSQD_SDM", surface=TRUE), ask = FALSE)
```  
...
plot(conditional_effects(
  fit_qPSDN_gamma_v2, "PSDN_SDM", surface=TRUE, conditions = conditions, 
...
```{r by_port_msqd_sdm}
plot(conditional_effects(
  fit_qPSDN_gamma_v2, "MSQD_SDM", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
...
```{r by_port_acl_psdn}
plot(conditional_effects(
  fit_qPSDN_gamma_v2, "ACL_PSDN", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
...
```{r int_effect_total, echo=FALSE, fig.cap=, message=FALSE, warning=FALSE}

# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qPSDN_gamma_v2, "PSDN_SDM:MSQD_SDM", surface=TRUE), ask = FALSE)
...
plot(conditional_effects(
  fit_qPSDN_gamma_v2, "PSDN_SDM:MSQD_SDM", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
```
...
est_data_msqd_all <- est_data_msqd_all %>%
  ...
  mutate(dOpen = ifelse(Landing_year < 2015,1,0))
...
est_data_msqd_all$f1f2 <- interaction(est_data_msqd_all$dClose, est_data_msqd_all$port_ID, drop = TRUE)
est_data_msqd_all <- est_data_msqd_all %>%
                      mutate(PSDN_SDM_dOpen = PSDN_SDM * dOpen)

fit_qMSQD_gamma_dClose_v7 <- brm(
bf(MSQD_Landings ~ s(MSQD_SDM, PSDN_SDM_dOpen) + dClose + (1 + MSQD_SDM + PSDN_SDM_dOpen + dClose | port_ID),
              hu ~ s(MSQD_SDM, PSDN_SDM_dOpen) + dClose + (1 + MSQD_SDM + PSDN_SDM_dOpen + dClose | port_ID)),
                       data = est_data_msqd_all,
                       family = hurdle_gamma(),
                       control = list(adapt_delta = 0.999, max_treedepth = 20),
                       chains = 2, cores = 4)

fit_qMSQD_gamma_dClose_v5 <- brm(
 bf(MSQD_Landings ~ MSQD_SDM + PSDN_SDM_dOpen + dClose + (1 + MSQD_SDM + PSDN_SDM_dOpen + dClose | port_ID),
               hu ~ MSQD_SDM + PSDN_SDM_dOpen + dClose + (1 + MSQD_SDM + PSDN_SDM_dOpen + dClose | port_ID)),
                       data = est_data_msqd_all,
                       family = hurdle_gamma(),
                       control = list(adapt_delta = 0.999, max_treedepth = 20),
                       chains = 2, cores = 4)
...
LOO(fit_qMSQD_gamma_dClose_v5, fit_qMSQD_gamma_dClose_v7)
...
class(est_data_msqd_all$port_ID)
pp_check(fit_qMSQD_gamma_dClose_v5) + ggtitle('Gamma distribution + dClose') + theme(legend.position = "bottom")
```
...
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qMSQD_gamma_dClose_v5, "PSDN_SDM_dOpen", surface=TRUE), ask = FALSE)
```  
...
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qMSQD_gamma_dClose_v5, "MSQD_SDM", surface=TRUE), ask = FALSE)
```  

```{r ACL_effect_msqd, echo=FALSE, fig.cap="Effect ACL on MSQD", message=TRUE, warning=TRUE}
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qMSQD_gamma_dClose_v5, "dClose"), ask = FALSE)
```  
...
plot(conditional_effects(
  fit_qMSQD_gamma_dClose_v5, "PSDN_SDM_dOpen", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
```
...
plot(conditional_effects(
  fit_qMSQD_gamma_dClose_v5, "MSQD_SDM", surface=TRUE, conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
...
plot(conditional_effects(
  fit_qMSQD_gamma_dClose_v5, "dClose", conditions = conditions, 
  re_formula = NULL), ncol = 4, points = FALSE)
...
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qMSQD_gamma_dClose_v4, "PSDN_SDM:MSQD_SDM", surface=TRUE), ask = FALSE)
