...
On average, vessels land their catch in 4.8 ports, with a standard deviation of 3.94. If we only consider CPS landings, vessels land in 2.1 ports on average, with a standard deviation of 2.17. If we disaggregate this information by species, vessels land on average market squid in 2.25 ports, Pacific sardine in 2.25 ports, Northern anchovy in 1.61 ports, and Pacific mackerel in 1.51 ports. 

**<<<<<<<<<<<CONTINUE HERE>>>>>>>>>>>>> **
<!-- The ability to switch using seine as gear is confirmed when we analyze the most common gears used by different clusters (see Table \ref{Table:cluster_port}). In Cluster 5, the cluster where we observe substitution, 88\% of the cluster's total landings correspond to catches using seine as a gear. Only 7\% and 3\% of the cluster's total landings correspond to catches using other net gear and midwater trawl, respectively. -->
...
print(xtable(table, caption = 'Percentage of cluster total langings by gear used.\\label{Table:cluster_port}', type = "latex"), comment=FALSE,  caption.placement = "top")

rm(table)

```

<< CHECK PORT BY CLUSTER >>
...
<!-- "Weather and tides are major factors in fishing operations and timing of vessels transiting in and out of the Columbia River." SAFE -->
