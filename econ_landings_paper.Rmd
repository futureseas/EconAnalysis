...
est_data_psnd$port_ID<- factor(est_data_psnd$port_ID)
str(est_data_psnd$port_ID)
...
str(est_data_msqd_all$dClose)
est_data_msqd_all$port_ID<- factor(est_data_msqd_all$port_ID)
str(est_data_msqd_all$port_ID)

fit_qMSQD_gamma_dClose <- brm(
  bf(MSQD_Landings ~ s(PSDN_SDM, MSQD_SDM, by = port_ID) + (dClose +  1 | port_ID),
                hu ~ s(PSDN_SDM, MSQD_SDM, by = port_ID) + (dClose +  1 | port_ID)),
...
fit_qMSQD_lognormal <- add_criterion(fit_qMSQD_gamma_dClose, "loo")
...
pp_check(fit_qMSQD_gamma_dClose) + ggtitle('Lognormal distribution') + theme(legend.position = "bottom")
```
...
# summary(fit_qPSDN_gamma) 
plot(conditional_effects(fit_qMSQD_gamma_dClose, "port_ID"), ask = FALSE)
```  
