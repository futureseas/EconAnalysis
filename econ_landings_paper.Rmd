...
We conduct a cluster analysis to categorize better the different portfolio that fisher manage during a year. For this we use a hierarchical clustering. This technique have been used before in fisheries... This approach do not require to define de number of cluster as is chosen within the algorithm. 
...
PacFIN.spread[is.na(PacFIN.spread)] <- 0
clusters <- hclust(dist(PacFIN.spread[, 2:5]), method = 'average')
plot(clusters)
clusterCut <- as.data.frame(cutree(clusters, 4))
PacFIN.spread$cluster <- clusterCut$`cutree(clusters, 4)`
