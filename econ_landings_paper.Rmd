...
institute:
  ...
  - noaa2: NOAA Northhwest Fisheries Science Center
  - noaa3: NOAA Pacific Island Fisheries Science Center
  
date: "`r format(Sys.time(), '%B %e, %Y')`"
abstract: "Many fisheries are not single but multispecies fisheries. Fisher does not only catch one species. They have a set of possible choices, called fishing portfolios, that allow them to diversify and reduce income risk. For some species, switching between them is more accessible as they share similar gear and methods for fishing. Then, fishers could adapt quickly to changes in species spatial distributions as a consequence of climate change. However, regulations and other constraints (e.g., port constraints for landing) may reduce the degree of substitution we observe. In this study, we analyze how changing species distribution over time, and the closure of the  Pacific sardine fishery affects landing substitution between three coastal pelagic species: Pacific sardine, market squid, and Northern anchovy. Using a discrete choice modeling approach, we also study how spatial distribution and closure affect the CPS fishery's participation decisions. We use species distribution model projections to study how landings and participations change under different future climate change scenarios. Our results show strong substitution between market squid and Pacific sardine, and the Pacific sardine closure in 2015 reduced market squid landings."
header-includes:
...
Some communities are more vulnerable to change than other as the set of species choices is reduced by port constraints in capacity and switching capacity due to heterogeinety of fleets (FROM CPS Workshop). Processor constraints may also reduce switching capacity. " But there is a limit to how far its profitable to truck
...
infrastructure could mean that squid goes unharvested." (CPS Workshop)
...
g2 <- ggplot(revenue.avg.year, aes(PACFIN_SPECIES_CODE, AFI_EXVESSEL_REVENUE.sum.sum.mean)) +
...
rm(g1, g2, landings.year, landings.avg.year, revenue.year, revenue.avg.year)
```


```{r ts_landings, echo=FALSE, fig.cap="Total annual landing by CPS species (1981 - 2020).\\label{fig:ts_landings}", message=FALSE, warning=FALSE}
...
Fom the 1st WORKSHOP with Stakeholder about Sardine: "There was a discussion on fisher behavior and what other species sardine vessels target, particularly when the sardine fishery is closed. A participant stated that in California, to lessen the impact of the sardine closure, many vessels switched to market squid." However, this is nit reflected in the graph, and participation may decline... MAybe also relative prices with other species. The workshop say that substitution may happen also with mackerel, tuna, or anchovy **<<Should we include tuna and mackerel in the analysis??>>** 

Also the workshop say that squid is now more year around after pacific sardine closure. We can test this....



```{r ts_landing_price, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r ts_landings_vessels, echo=FALSE, message=FALSE, warning=FALSE}
...
# Graph landing v/s number of  vessels
coeff <- 1000
  df <- landing.nvessels.year.sel %>% filter(PACFIN_SPECIES_CODE == "MSQD")
...
coeff <- 1000
  df <- landing.nvessels.year.sel %>% filter(PACFIN_SPECIES_CODE == "PSDN")
  df$n_vessel <- df$n_vessel * coeff 
  df <- gather(df, key = Variable, value = value,
               c("LANDED_WEIGHT_MTONS.sum.sum", "n_vessel"))
  g2 <- ggplot(df, aes(x=LANDING_YEAR, y = value, colour = Variable)) + geom_line(size=1) +
    scale_y_continuous(name = "", sec.axis = sec_axis(~./coeff, name = "Number of vessels")) + 
    theme(legend.position="none") + scale_x_continuous(name = "") +
    theme(plot.title = element_text(size=9, face="bold.italic"), 
          axis.text = element_text(size = 7), axis.title = element_text(size = 8)) + 
      ggtitle("(b) Pacific Sardine")  +  
...
coeff <- 30
  df <- landing.nvessels.year.sel %>% filter(PACFIN_SPECIES_CODE == "PHRG")
  df$n_vessel <- df$n_vessel * coeff 
  df <- gather(df, key = Variable, value = value,
               c("LANDED_WEIGHT_MTONS.sum.sum", "n_vessel"))
  g3 <- ggplot(df, aes(x=LANDING_YEAR, y = value, colour = Variable)) + geom_line(size=1) +
  ...
  theme(plot.title = element_text(size=9, face="bold.italic"), 
        axis.text = element_text(size = 7), axis.title = element_text(size = 8)) +
    ggtitle("(c) Pacific Herring")  + 
    scale_color_brewer(palette="Set2", limits = c("LANDED_WEIGHT_MTONS.sum.sum", "n_vessel"))

coeff <- 500
  df <- landing.nvessels.year.sel %>% filter(PACFIN_SPECIES_CODE == "NANC")
  df$n_vessel <- df$n_vessel * coeff 
  df <- gather(df, key = Variable, value = value,
               c("LANDED_WEIGHT_MTONS.sum.sum", "n_vessel"))
  g4 <- ggplot(df, aes(x=LANDING_YEAR, y = value, colour = Variable)) + geom_line(size=1) +
    ...
    ggtitle("(d) Northern Anchovy") + guides(colour=guide_legend(title="Variables: ")) + 
  scale_color_brewer(palette="Set2", limits = c("LANDED_WEIGHT_MTONS.sum.sum", "n_vessel"),
                     labels=c("n_vessel" = "Number of vessels", "LANDED_WEIGHT_MTONS.sum.sum" = "Landings"))
...
<!-- *Jan 1 2000, federal harvest guidelines from CPS FMP first implemented				 -->
<!-- Implmentation of the CA squid management plan in 2005 reduced the fleet to a max of 73 (55 vessels and 18 brail vessels, also permits limits light boats to 34). The fishery was unregulated and open access until permit became required in 1998, but no entry limits until 2005. A HG was first implemented in 2001				 -->
<!-- California peak sardine harvest season is in fall and early winter. Monterey focuses on larger sardine in early summer-fall on their southern migraiton, Southern CA peak harvest occurs late fall through Jan-Feb --Let's check this with the data				 -->
<!-- In 2005 saridne allocation was modified from area-based to a coast-wide seasonal release				 -->
<!-- The CA market squid fishery in the northern CA (north of Pt. Conception, Monterey Bay focus) peaks in summer, in southern CA in late fall/early winter.				 -->
<!-- The market squid fishery has a weekend closure				 -->
<!-- Note that in CDFW MSFMP, the HG in reported in short tons (125000 in 2001 and 118000 in 2004)				 -->
<!-- Since 1999 a fishery for sardine has operated in OR/WA targeting larger sardine sold as bait to longliners, in 2006 fishery expanded into human consumption markets				 -->
<!-- CA moratorium on sardine in 1967 following collapse, only reopened in 1986 with a very precautionary 907 mt quota				 -->
<!-- HG information for sardine from SAFE reports (2000 onwards) and Wolf 1992 for 1981-1992 and Barnes et al. 1996 for 1996				 -->
<!-- Squid information from CDFW MSFMP				 -->
<!-- OR sardine fishery was a developmental fishery from 1999 to 2005, in 2006 first LE permits issued				 -->
<!-- WA sardine fishery was a trial fishery from 2000-2002 with a state HG of 15000 mt, from 2003-2009 it was experimental with permits capped at 25. It was designated a commercial fishery in 2009 with a LE program with 16 permanent permits and 10 temporary ones. Fishign for sardine is prohibited Jan 1 - MArch 31, but fishing is limited in winter anyways because of weather and low fish availability				 -->
<!-- First OR market squid fishery post CPS FMP implementation in 2000 occured in 2014, but some landings in 1980s following El Nino events				 -->
<!-- Never directed landings of squid in WA but in 2019 two permits were issued (interest was not in directed catch but in ability to deliver OR squid to WA ports)				 -->

<!-- Peak Fishing Season (to be checked when data arrives)				 -->
<!-- 	           Spring	   Summer	 Fall	Winter  -->
<!-- WA	         Sardine	Sardine		            -->
<!-- OR	         Sardine	Sardine		            -->
<!-- Monterey		           Squid	Sardine	      -->
<!-- Southern CA			             Squid	Squid -->
<!-- Southern CA			            Sardine	Sardine -->
...
## Switching behavior
<!-- - Graficar número de barcos que desembarcan especies de CPS durante cada año/mes.  -->
<!-- - Graficar porcentaje de barcos que desembarcan squid condicional a que desembarcan sardina durante un año (o meses). (Y otras combinaciones) -->
<!-- - Desagregado por puerto? -->
...
```{r switching_quarterv2}
psdn.by.quarter <- PacFIN.month.CPS %>% filter(LANDING_YEAR >= 2000) %>% filter(LANDING_YEAR <= 2014) %>% 
  mutate(quarter = quarter(zoo::as.yearmon(paste(LANDING_YEAR, LANDING_MONTH, sep='-')), with_year = TRUE)) %>% 
  mutate(quarter2 = quarter(zoo::as.yearmon(paste(LANDING_YEAR, LANDING_MONTH, sep='-')), with_year = FALSE)) %>% 
  select(VESSEL_NUM, PACFIN_SPECIES_CODE, quarter, quarter2) %>% filter(PACFIN_SPECIES_CODE  %in% c("PSDN")) %>% 
  unique() %>% mutate(harv.psdn = 1) 
...
# Calculate number of vessels harvesting sardine and squid and percentage per year
n_vessel_psdn <- psdn.by.quarter %>% group_by(quarter, quarter2) %>% 
  summarize(n_vessel_psdn = sum(harv.psdn, na.rm = TRUE)) %>% group_by(quarter2) %>% 
  summarize(n_vessel_psdn = mean(n_vessel_psdn, na.rm = TRUE))

n_vessel_msqd_c_psdn <- squid.cond.psdn %>% group_by(quarter, quarter2) %>% 
  summarize(n_vessel_msqd = sum(harv.psdn, na.rm = TRUE)) %>% group_by(quarter2) %>% 
  summarize(n_vessel_msqd = mean(n_vessel_msqd, na.rm = TRUE))
  
df <- merge(n_vessel_psdn, n_vessel_msqd_c_psdn, by=c("quarter2"), all.x = TRUE) %>%
    mutate(n_vessel_msqd = if_else(is.na(n_vessel_msqd), 0, n_vessel_msqd)) %>%
    mutate(perc = (n_vessel_msqd / n_vessel_psdn)) %>% select(quarter2, perc)

ggplot(data=df, aes(x=quarter2, y=perc)) +
    geom_bar(stat="identity", fill=rgb(0.1,0.4,0.5,0.7), width=0.4) + 
  scale_x_continuous(name = "Quarter", breaks=1:4, labels=c("Q1","Q2","Q3","Q4")) +
...
n_vessel_psdn <- psdn.by.month %>% group_by(LANDING_YEAR, LANDING_MONTH) %>% 
  summarize(n_vessel_psdn = sum(harv.psdn, na.rm = TRUE)) %>% group_by(LANDING_MONTH) %>% 
  summarize(n_vessel_psdn = mean(n_vessel_psdn, na.rm = TRUE))

n_vessel_msqd_c_psdn <- squid.cond.psdn %>% group_by(LANDING_YEAR, LANDING_MONTH) %>% 
  summarize(n_vessel_msqd = sum(harv.psdn, na.rm = TRUE)) %>% group_by(LANDING_MONTH) %>% 
  summarize(n_vessel_msqd = mean(n_vessel_msqd, na.rm = TRUE))
  
df <- merge(n_vessel_psdn, n_vessel_msqd_c_psdn, by=c("LANDING_MONTH"), all.x = TRUE) %>%
    mutate(n_vessel_msqd = if_else(is.na(n_vessel_msqd), 0, n_vessel_msqd)) %>%
...
## Management
"Pacific sardine is managed under a permit and quota management system, with a HG controlling commercial catch of sardine biomass."
<< DOES FISHER HAVE TO BUY OR SELL QUOTA LIKE IN AN ITQ SYSTEM? IS THERE A FINE FOR EXCEDING THE QUOTA, IT IS INDIVIDUAL QUOTA >>
...
PacFIN.month.dataset <- PacFIN.month.dataset %>%   filter(LANDING_YEAR >= 2000 & LANDING_YEAR <= 2018) %>%
  ...
  mutate(port_num = as.numeric(as.factor(PORT_NAME))) %>%
...
sjlabelled::set_label(dataset$MSQD_LANDED_WEIGHT_MTONS.sum) <- "Landings: MSQD"
sjlabelled::set_label(dataset$MSQD_AFI_PRICE_PER_KG.mean)   <- "Price: MSQD"
sjlabelled::set_label(dataset$NANC_LANDED_WEIGHT_MTONS.sum) <- "Landings: NANC"
sjlabelled::set_label(dataset$NANC_AFI_PRICE_PER_KG.mean)   <- "Price: NANC"
sjlabelled::set_label(dataset$PSDN_LANDED_WEIGHT_MTONS.sum) <- "Landings: PSDN"
sjlabelled::set_label(dataset$PSDN_AFI_PRICE_PER_KG.mean)   <- "Price: PSDN"
sjlabelled::set_label(dataset$ACL_PSDN)      <- "Annual Catch Limit: PSDN"
sjlabelled::set_label(dataset$port_num)      <- "Port ID"

desc_data <- dataset %>%
  subset(select = -c(port_num, LANDING_YEAR, LANDING_MONTH, VESSEL_NUM, 
...
sjlabelled::set_label(desc_data$MSQD_LANDED_WEIGHT_MTONS.sum) <- "Landings: MSQD"
sjlabelled::set_label(desc_data$MSQD_AFI_PRICE_PER_KG.mean)   <- "Price: MSQD"
sjlabelled::set_label(desc_data$NANC_LANDED_WEIGHT_MTONS.sum) <- "Landings: NANC"
sjlabelled::set_label(desc_data$NANC_AFI_PRICE_PER_KG.mean)   <- "Price: NANC"
sjlabelled::set_label(desc_data$PSDN_LANDED_WEIGHT_MTONS.sum) <- "Landings: PSDN"
sjlabelled::set_label(desc_data$PSDN_AFI_PRICE_PER_KG.mean)   <- "Price: PSDN"
sjlabelled::set_label(desc_data$ACL_PSDN)      <- "Annual Catch Limit: PSDN"


desc_data <- desc_data[c("PSDN_SDM_60", "MSQD_SDM_90", "NANC_SDM_20",
"PSDN_LANDED_WEIGHT_MTONS.sum",
"MSQD_LANDED_WEIGHT_MTONS.sum",
"NANC_LANDED_WEIGHT_MTONS.sum",
"PSDN_AFI_PRICE_PER_KG.mean",
"MSQD_AFI_PRICE_PER_KG.mean",
"NANC_AFI_PRICE_PER_KG.mean",
"ACL_PSDN")]
