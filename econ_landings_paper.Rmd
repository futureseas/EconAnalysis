...
## Clusters within CPS fleet

```{r only_psdn, include=FALSE}
psdn.hist <- PacFIN.month.CPS %>% filter(LANDING_YEAR >= 2000) %>% filter(LANDING_YEAR <= 2014) %>% 
  select(VESSEL_NUM, PACFIN_SPECIES_CODE) %>% filter(PACFIN_SPECIES_CODE  %in% c("PSDN")) %>% 
  unique() %>% mutate(harv.psdn = 1)

msqd.hist <- PacFIN.month.CPS %>% filter(LANDING_YEAR >= 2000) %>% filter(LANDING_YEAR <= 2014) %>% 
  select(VESSEL_NUM, PACFIN_SPECIES_CODE) %>% filter(PACFIN_SPECIES_CODE  %in% c("MSQD")) %>% 
  unique() %>% mutate(harv.msqd = 1)

# Which are the vessel that harvest both PSDN and MSQD?
squid.and.psdn <- PacFIN.month.CPS %>% filter(LANDING_YEAR >= 2000) %>% filter(LANDING_YEAR <= 2014) %>% 
  select(VESSEL_NUM, PACFIN_SPECIES_CODE) %>% merge(psdn.hist, 
                        by = c("VESSEL_NUM"), all.y = TRUE) %>% filter(harv.psdn == 1) %>%
  filter(PACFIN_SPECIES_CODE.x == "MSQD") %>% unique() %>% select(VESSEL_NUM) %>%
  mutate(d_PSDN_and_MSQD = 1)

# Which are the vessel that harvest only MSQD?
only.squid <- PacFIN.month.CPS %>% filter(LANDING_YEAR >= 2000) %>% filter(LANDING_YEAR <= 2014) %>% 
  select(VESSEL_NUM, PACFIN_SPECIES_CODE) %>% merge(squid.and.psdn, by = c("VESSEL_NUM"), all.x = TRUE) %>%
  filter(PACFIN_SPECIES_CODE == "MSQD") %>% filter(is.na(d_PSDN_and_MSQD)) %>% unique() %>% select(VESSEL_NUM) %>%
  mutate(d_onlyMSQD = 1)

# Which are the vessel that harvest only PSDN?
only.psdn <- PacFIN.month.CPS %>% filter(LANDING_YEAR >= 2000) %>% filter(LANDING_YEAR <= 2014) %>% 
  select(VESSEL_NUM, PACFIN_SPECIES_CODE) %>% merge(squid.and.psdn, by = c("VESSEL_NUM"), all.x = TRUE) %>%
  filter(PACFIN_SPECIES_CODE == "PSDN") %>% filter(is.na(d_PSDN_and_MSQD)) %>% unique() %>% select(VESSEL_NUM) %>%
  mutate(d_onlyPSDN = 1)
...
df <- PacFIN.month %>% filter(LANDING_YEAR >= 2000) %>% filter(LANDING_YEAR <= 2014) %>% 
  merge(squid.and.psdn, by = c("VESSEL_NUM"), all.x = TRUE) %>% filter(d_PSDN_and_MSQD == 1) 

# rm(psdn.hist, squid.hist, only.squid, only.psdn, squid.and.psdn)
