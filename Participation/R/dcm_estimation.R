...
# %>%
#   filter(selection != "LAA-YTNA") %>% filter(selection != "LAA-BTNA")
...
model <- mlogit(fished ~ mean_price + mean_avail + wind_max_220_mh + dist_to_cog + dist_port_to_catch_area_zero +
                  PSDN.Closure.d + d_d + d_cd + unem_rate | 1 + Weekend, 
                data = the_tows,
