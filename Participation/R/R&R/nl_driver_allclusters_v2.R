# Auto-generated driver: NL SDM availability robustness across clusters c4-c7
...
build_database <- function(long_data_spec, alts, case_vars){
  ...
  long_data_spec <- long_data_spec %>%
    ...
    mutate(
      unem_rate_case = { v <- unem_rate[selection!="no_participation" & !is.na(unem_rate)];
...
run_cluster <- function(cluster_id, rds_file, alts, apollo_beta_base, apollo_fixed_base,
                        ...
                        use_d_c_by_spec = c(daily=1, MA7=1, MA14=1, MA30=1, t1daily=1)){
  message("\n===== Running ", cluster_id, " =====")
  ...
  long_data2 <- long_data %>%
    ...
    left_join(cpue_expanded, by=c("PORT_AREA_CODE"="PORT_AREA_CODE","date"="date","sp4"="Species_Dominant")) %>%
    mutate(CPUE_avail_t1 = coalesce(CPUE_MA30_t1, CPUE_MA90_t1))
...
apollo_probabilities_c4 <- function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[["sba_msqd"]] <- asc_sba_msqd + w_msqd * weekend + B_mean_avail*mean_avail_sba_msqd + B_mean_price*mean_price_sba_msqd + B_wind_max_220_mh*wind_max_220_mh_sba_msqd + B_d_d*d_d_sba_msqd + B_d_cd*d_cd_sba_msqd + (B_d_c*use_d_c)*d_c_sba_msqd + B_dist_port_to_catch_area_zero*dist_port_to_catch_area_zero_sba_msqd + B_dummy_prev_days*dummy_prev_days_sba_msqd + B_dummy_prev_year_days*dummy_prev_year_days_sba_msqd + B_unem_rate_part*unem_rate + B_dist_to_cog*dist_to_cog_sba_msqd
...
# ---- Run all clusters ----
use_d_c_by_spec <- c(daily=1, MA7=1, MA14=1, MA30=1, t1daily=1)  # tweak if needed
