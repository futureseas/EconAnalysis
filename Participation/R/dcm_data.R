...
samps1 <- sampled_rums(data_in = participation_data, cluster = 2,
  ...
  samps <- samps1 %>%
    ...
    rm(samps1)
    saveRDS(samps, file = "C:\\Data\\PacFIN data\\sample_choice_set_c2_full.rds")

### Run saved data ####
samps <- readRDS(file = "C:\\Data\\PacFIN data\\sample_choice_set_c2_full.rds")
...
write.csv(rdo_Stata,"C:\\Data\\PacFIN data\\rdo_Stata_c2_full.csv", row.names = FALSE)
saveRDS(rdo_Stata, file = "C:\\Data\\PacFIN data\\rdo_Stata_c2_full.rds")
...
## Save data to run with Stata
write.csv(rdo_Stata_noid,"G:\\My Drive\\Data\\Anonymised data\\rdo_Stata_c2_full_noid.csv", row.names = FALSE)
