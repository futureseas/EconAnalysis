...
for (y in 2000:2018) {
  for (m in 1:12) {
    for (j in 1:nrow(port_area_coord)) {
      ...
      dat <- ncdf4::nc_open(paste0("G:/My Drive/Data/SDM/jack/jack_",
      ...
      jmck.date.sdm <- ncdf4::ncvar_get(dat, "time")
      jmck.sdm <- ncdf4::ncvar_get(dat, "predGAM")
      ncdf4::nc_close(dat)			
      ...
      sdmMelt <- sdmMelt %>%
        ...
        summarize(exp_prob = mean(jmck.sdm, na.rm = T))	%>%
        ungroup(.)  
