...
nlogitgen partp = port(PART: MSQD | PSDN | ALBC | CMCK | NANC, NOPART: NOPORT)
...
nlogit fished mean_avail mean_price2 wind_max_220_mh dist_to_cog dist_port_to_catch_area_zero ///
		psdnclosured dummy_last_day unem_rate d_d d_pd d_cd d_pcd dcrbclosurewad /// 
		|| partp: , base(NOPART) || port: weekend, base(NOPORT) || selection: , ///
	base("No-Participation") case(fished_haul) constraints(1) vce(cluster fished_vessel_id) ///
	from(start, skip)
matrix start=e(b)
...
// + WA closure for sardine (problem: backed-up last iteration)
nlogit fished mean_avail mean_price2 wind_max_220_mh dist_to_cog dist_port_to_catch_area_zero ///
		psdnclosured dummy_last_day unem_rate d_d d_pd d_cd d_pcd dcrbclosurewad waclosured  /// 
		|| partp: , base(NOPART) || port: weekend, base(NOPORT) || selection: , ///
	base("No-Participation") case(fished_haul) constraints(1) vce(cluster fished_vessel_id) ///
	from(start, skip)
matrix start=e(b)
...
// + Squid closure (problem: backed-up last iteration)
nlogit fished mean_avail mean_price2 wind_max_220_mh dist_to_cog dist_port_to_catch_area_zero ///
...
// + constraint 2 (SOLVED PROBLEM WITH BACKED-UP ITERATION -- check gradient to see if zero)
nlogit fished mean_avail mean_price2 wind_max_220_mh dist_to_cog dist_port_to_catch_area_zero ///
		psdnclosured dummy_last_day unem_rate d_d d_pd d_cd d_pcd dcrbclosurewad waclosured msqdclosured /// 
		|| partp: , base(NOPART) || port: weekend, base(NOPORT) || selection: , ///
...
nlogit fished mean_avail mean_price2 wind_max_220_mh dist_to_cog dist_port_to_catch_area_zero ///
		psdnclosured dummy_prev_days unem_rate msqdclosured dcrbclosurewad waclosured d_c d_d d_p d_cd d_pc d_pd d_pcd  /// 
		|| partp: , base(NOPART) || port: weekend, base(NOPORT) || selection: , ///
...
estimates store B8
estimates describe B8
estimates replay B8
di "R2-McFadden = " 1 - (e(ll)/ll0)
estadd scalar r2 = 1 - (e(ll)/ll0): B8
lrtest base B8, force
estadd scalar lr_p = r(p): B8
estat ic, all
matrix S = r(S)
estadd scalar aic = S[1,5]: B8
estadd scalar bic = S[1,6]: B8
estadd scalar aicc = S[1,7]: B8
estadd scalar caic = S[1,8]: B8
preserve
	qui predict phat
	by fished_haul, sort: egen max_prob = max(phat) 
	drop if max_prob != phat
	...
	egen count1 = total(fished)
	dis count1/_N*100 "%"
	estadd scalar perc1 = count1/_N*100: B8
	drop if selection == "No-Participation"
	egen count2 = total(fished)
	dis _N
	dis count2/_N*100 "%"
	estadd scalar perc2 = count2/_N*100: B8
restore
preserve
	replace dummy_last_day = 0
	replace dummy_prev_days = 0
	...
	egen count1 = total(fished)
	dis count1/_N*100 "%"
	estadd scalar perc3 = count1/_N*100: B8
	drop if selection == "No-Participation"
	egen count2 = total(fished)
	dis _N
	dis count2/_N*100 "%"
...
esttab  B4 B5 B6 using "G:\My Drive\Tables\Participation\nested_logit_FULL_${S_DATE}_C5.rtf", ///
