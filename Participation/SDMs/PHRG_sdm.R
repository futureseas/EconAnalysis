...
saveRDS(coord, "Participation/SDMs/port_dist_phrg/coord_sdm_phrg.rds")


#------------------------------------------------------
# Get distance by coordinate to port j and save it

coord <- readRDS("Participation/SDMs/port_dist_phrg/coord_sdm_phrg.rds")
...
for (j in 1:nrow(port_area_coord)) {
  ...
  saveRDS(distPorts, paste0("Participation/SDMs/port_dist_phrg/portDist_", paste0(as.character(j),".rds")))
}
...
for (y in 2000:2018) {
  for (m in 1:12) {
    for (j in 1:nrow(port_area_coord)) {
      ...
      distPorts <- readRDS(paste0("Participation/SDMs/port_dist_phrg/portDist_",
      ...
      for (z in 1:max(sdmMelt$LANDING_DAY)) {
        ...
        SDM_mean_20 <- mean(dat_prob_20$phrg.sdm, na.rm = TRUE)
        sdm.phrg <- sdm.phrg %>%
          add_row(LANDING_YEAR = y, 
                  ...
                  PHRG_SDM_20 = SDM_mean_20)
        rm(dat_prob_30, dat_prob_90, dat_prob_20, 
           SDM_mean_30, SDM_mean_90, SDM_mean_20)
      }
      print(paste("Year:", y, "; month:", m, "--", "Port area:",j))
      saveRDS(sdm.phrg, file = "Participation/SDMs/sdm_phrg.rds")
    }
  }
}
