...
### Set core controls
apollo_control = list(
  modelName       = "NL_participation_model_c4_30days",
...
long_data = readRDS(paste0(google_dir, "Data/Anonymised data/part_model_c4.rds")) %>%
  dplyr::select("fished_haul_anon", "fished_vessel_anon", "selection", "fished",
                ...
                "unem_rate", "weekend", "d_missing_p2", "d_missing", "d_missing_d") 
...
long_data <- long_data %>%
  ...
  dplyr::rename(mean_price = mean_price2)
...
summary(df_selected) # d_pcd_laa_ytna
df_selected <- database %>%
  dplyr::select(starts_with("d_d_"))
summary(df_selected) # all
df_selected <- database %>%
  dplyr::select(starts_with("d_pd_"))
summary(df_selected) # d_pd_laa_msqd d_pd_laa_ytna d_pd_mna_msqd d_pd_sba_msqd d_pd_mra_msqd     d_pd_laa_psdn d_pd_laa_btna    d_pd_sfa_msqd     d_pd_mna_psdn
df_selected <- database %>%
  dplyr::select(starts_with("d_pcd_"))
summary(df_selected) ### d_pcd_laa_ytna d_pcd_laa_btna
...
# apollo_beta=apollo_readBeta(apollo_beta,apollo_fixed,"NL_participation_model_c4_30days",overwriteFixed=FALSE)
