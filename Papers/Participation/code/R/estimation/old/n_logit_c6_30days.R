...
### Set core controls
apollo_control = list(
  modelName       = "NL_participation_model_c6_30days",
...
long_data <- readRDS(paste0(google_dir, "Data/Anonymised data/part_model_c6.rds")) %>%
  dplyr::select("fished_haul_anon", "fished_vessel_anon", "selection", "fished",
                "fished_vessel_anon", "mean_avail", "mean_price2", "mean_price_annual", 
                ...
                "d_missing_p2", "d_missing", "d_missing_d", "weekend") 
...
long_data <- long_data %>%
  dplyr::mutate(
    mean_price2_3 = ifelse(selection == "cwa_dcrb" | selection == "nps_sock", mean_price_annual, mean_price2)) %>% 
...
database <- long_data %>%
  pivot_wider(
    ...
    values_from = c(fished, mean_avail, mean_price_3, wind_max_220_mh, dist_to_cog, 
                    ...
                    dcrbclosurewad, waclosure, unem_rate, d_d, d_pd, d_pcd)) %>%
...
apollo_beta=c(B_mean_avail                   = 2.3,
              ...
              B_dist_to_cog                  = -0.005,
              B_dist_port_to_catch_area_zero = -0.002,
              B_dummy_prev_days              = 2,
              ...
              B_d_pd                         = -0.5,
              B_d_pcd                        = -0.5,
              B_mean_price_part              = 0.15,
              B_mean_price_crab              = 0.15,
              B_mean_price_slmn              = 3,
              w_cmck                         = -0.1,
              w_psdn                         = -3, 
              c_psdn                         = -3,
...
# apollo_beta=apollo_readBeta(apollo_beta,apollo_fixed,"NL_participation_model_c6_30days",overwriteFixed=FALSE)
