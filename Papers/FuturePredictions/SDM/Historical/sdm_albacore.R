...
with_progress({
  results_list <- future_map(
    ...
    function(ym) {
      y <- ym$year
      m <- ym$month
      nc_path <- sprintf("C:/Data/Historical CPS SC-GAMs ROMS Domain/albacore/alb_%d_%d_mboost_roms.nc", m, y)
      ...
      result <- sdm_dt[, .(SDM_90 = mean(pred, na.rm = TRUE)), by = .(date, PORT_AREA_CODE)]
      result[, `:=`(
...
fwrite(SDM_all, here::here("FuturePredictions", "SDM", "Historical", "ALBC_SDM_port_day.csv"))
