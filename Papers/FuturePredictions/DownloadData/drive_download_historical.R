...
folder_id <- as_id("1cSUc2DAAdvFQPpiwJKJ60fxpdYZJJXJ9")
...
all_files <- get_all_files_recursive("1cSUc2DAAdvFQPpiwJKJ60fxpdYZJJXJ9")
...
future_walk(
  file_chunks,
  download_chunk,
  dest_dir = "C:/Data/Historical CPS SC-GAMs ROMS Domain",
...
if (length(failed_downloads) > 0) {
  for (f in failed_downloads) {
    ...
    local_path <- path("C:/Data/Historical CPS SC-GAMs ROMS Domain", f$name)
