...
process_dummys2 <- function(xx, td2 = td1, dat1 = dat, hab_dist = 5, n_cost){
  ...
  dum30 <- dat1 %>% ungroup %>% dplyr::filter(haul_id != temp_dat$haul_id, 
  ...
  # #-----------------------------------------------------------------------------------------------
  # ## Calculate revenues (obtained by all the fleet withn 30 days, excluding the actual haul)
  ...
  # # NOTE: Here control whether you use value of all species or just target and groundfish species
